			...
			if (DialogueBlueprint->GeneratedClass)
			{
				if (UDialogue* DialogueCDO = Cast<UDialogue>(DialogueBlueprint->GeneratedClass->GetDefaultObject()))
				{
					for (auto& Speaker : DialogueCDO->Speakers)
					{
						...
						{
							FString OptionString = FString::Printf(TEXT("Add dialogue line for %s."), *Speaker.NPCDataAsset->NPCName.ToString());
...
void UDialogueGraphSchema::SetNodePosition(UEdGraphNode* Node, const FVector2f& Position) const
{
...
void UDialogueGraphSchema::GetHoverMessage(const TArray<struct FAssetData>& Assets, FString& OutTooltipText, bool& OutOkIcon) const 
{
	...
	{
		if (UObject* AssetObject = Asset.GetAsset(); AssetObject && AssetObject->IsA<USoundWave>())
		{
			NumSoundWaves++;
		}
	}
