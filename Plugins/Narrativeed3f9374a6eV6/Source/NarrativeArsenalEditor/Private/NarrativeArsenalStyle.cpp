...
constexpr FLinearColor NarrativeGreen = FLinearColor(0, 0.843f, 0.454f, 1.0f);
...
void FNarrativeArsenalStyle::Initialize()
{
	...
	StyleSet->Set(FName(TEXT("NarrativeToolbar.Icon")), new IMAGE_BRUSH("NarrativeLogo", FVector2D(64.f, 64.f), NarrativeGreen));
	StyleSet->Set(FName(TEXT("NarrativeToolbar.Icon.Small")), new IMAGE_BRUSH("NarrativeLogo", FVector2D(32.f, 32.f), NarrativeGreen));

	
	FSlateBrush LabelBrushNormal = *FAppStyle::Get().GetBrush("Icons.Edit");
	FSlateBrush LabelBrushHovered = LabelBrushNormal;
	LabelBrushHovered.TintColor = COLOR("18A0FBFF");
	StyleSet->Set("NarrativeMenu.Button.LabelEdit", FButtonStyle()
		.SetNormal (LabelBrushNormal)
		.SetHovered(LabelBrushHovered)
		.SetPressed(LabelBrushNormal)
		.SetNormalPadding(FMargin(0))
		.SetPressedPadding(FMargin(0))
		);

	FSlateBrush ShareBrushNormal = *FAppStyle::Get().GetBrush("ContentBrowser.Shared");
	FSlateBrush ShareBrushHovered = ShareBrushNormal;
	ShareBrushHovered.TintColor = COLOR("18A0FBFF");
	StyleSet->Set("NarrativeMenu.Button.Share", FButtonStyle()
		.SetNormal (ShareBrushNormal)
		.SetHovered(ShareBrushHovered)
		.SetPressed(ShareBrushNormal)
		.SetNormalPadding(FMargin(0))
		.SetPressedPadding(FMargin(0))
		);
	
	StyleSet->Set(FName(TEXT("NarrativeMenu.Button.PlayUsingSave")), new IMAGE_BRUSH_SVG("Play", FVector2D(24.0f, 24.0f), COLOR("18A0FBFF"))); // blue
