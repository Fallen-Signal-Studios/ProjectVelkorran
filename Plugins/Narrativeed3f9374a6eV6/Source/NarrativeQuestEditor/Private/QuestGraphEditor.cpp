		...
		if (UEdGraph* Graph = GraphEditor->GetCurrentGraph())
		{
			if (const UEdGraphSchema* Schema = Graph->GetSchema())
			{
				if (Schema->IsA(UEdGraphSchema_K2::StaticClass()) || Schema->IsA(UQuestGraphSchema::StaticClass())) 
				{
					...
					CommentAction.PerformAction(Graph, NULL, GraphEditor->GetPasteLocation2f());
				}
...
void FQuestGraphEditor::PasteNodesHere(class UEdGraph* DestinationGraph, const FVector2f& Location)
{
	if (UQuestGraph* QuestGraph = Cast<UQuestGraph>(DestinationGraph))
	{
		...
		for (TSet<UEdGraphNode*>::TIterator It(PastedNodes); It; ++It)
		{
			...
			PasteNode->NodePosX = static_cast<int32>((PasteNode->NodePosX - AvgNodePosition.X) + Location.X);
			PasteNode->NodePosY = static_cast<int32>((PasteNode->NodePosY - AvgNodePosition.Y) + Location.Y);
