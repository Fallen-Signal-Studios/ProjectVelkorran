...
public:
	...
	/** Should the navigation marker draw breadcrumbs */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Task - Navigation Markers")
	bool bDrawBreadcrumbs;

	/**The map marker class to use */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Task - Navigation Markers")
	TSubclassOf<class UMapMarker> MarkerClass;
	...
	//Subtitle to use for the marker. 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Task - Navigation Markers")
	FText MarkerSubtitleText;

	//The marker will be attached to this actor if you set a valid actor here. Will override any actor that the task provides. 
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Instanced, Category = "Task - Navigation Markers")
	TObjectPtr<UNarrativeActorProvider> MarkerAttachActorOverride;

	//Will be used as a fallback location if the actor we're trying to put the marker on doesn't exist yet. 
	UPROPERTY(EditAnywhere, Category = "Actor Provider")
	FVector ActorFallbackLocation;
...
public:
	...
	*/
	...
	/** Should we add a navigation marker to the world for the duration of this task */
	UPROPERTY()
	TObjectPtr<class UMapMarker> SpawnedMarker;
