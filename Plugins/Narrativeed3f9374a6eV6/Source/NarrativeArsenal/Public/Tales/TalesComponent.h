...
#include "TalesComponent.generated.h"
...
USTRUCT()
struct NARRATIVEARSENAL_API FSavedQuestBranch
{
	GENERATED_BODY()
...
USTRUCT()
struct NARRATIVEARSENAL_API FNarrativeSavedQuest
{
	GENERATED_BODY()
...
public:
	...
	//Raw serialized uproperties that live on the quest  
	UPROPERTY(SaveGame)
	TArray<uint8> QuestByteData;
...
*/
UCLASS( ClassGroup=(Narrative), DisplayName = "Tales Component", meta=(BlueprintSpawnableComponent) )
class NARRATIVEARSENAL_API UTalesComponent : public UActorComponent, public INarrativeSavableComponent
{
	GENERATED_BODY()
...
public:
	...
	/**When a tagged dialogue goes to to play it queries this to see if the cooldown passes. This prevents various tagged
	dialogues like Greetings to be spammed by characters ingame. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Dialogue")
	TMap<FGameplayTag, float> TaggedDialogueCooldowns;

	/**Fallback cooldown if tagged cooldown entry isnt added in map */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Dialogue")
	float FallbackCooldown;

	/**Caches the last time a tagged dialogue played on this client so we can apply cooldowns. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Dialogue")
	TMap<FGameplayTag, float> TaggedDialogueLastPlayTimes;
...
public:
	...
	*/
	...
	//Version used when loading from save. 
	virtual class UQuest* BeginQuest(const FNarrativeSavedQuest& SavedQuest);
...
public:
	...
	*/
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Quests", meta = (DeterminesOutputType = "QuestClass"))
	...
	*/
	...
	virtual void ClientReceiveSave(const TArray<FNarrativeSavedQuest>& InSavedQuests, const TArray<FString>& Tasks, const TArray<int32>& Quantities);
