...
public:
	...
	/**
	QUESTS ONLY - When the game loads back in, should we fire this event off again when your quest loads back in?
	...
	*/
	...
	/**
	Defines how events should be executed when the dialogue is playing as a party. Ignored if not in a party, will just run on solo component in that case. 
	*/
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Parties")
	...
	*/
	...
	void OnActivate(APawn* Target, APlayerController* Controller, class UTalesComponent* NarrativeComponent);
	...
	UFUNCTION(BlueprintNativeEvent, Category = "Event")
	void OnDeactivate(APawn* Target, APlayerController* Controller, class UTalesComponent* NarrativeComponent);
	...
	*/
	...
	void ExecuteEvent(APawn* Target, APlayerController* Controller, class UTalesComponent* NarrativeComponent);
	virtual void ExecuteEvent_Implementation(APawn* Target, APlayerController* Controller, class UTalesComponent* NarrativeComponent);
...
protected:
	...
	/**Characters we should run the event on. */
	...
	/** NPCs we should run the event on. */
	...
	/** Players we should run the event on. */
