...
APlayerController* UNarrativePartyComponent::GetOwningController() const
{
	...
	/*	TODO we should think a bit about whether this is exactly what we want, but for now parties run on dedi server
	 *	we use the party leader as the owning controller, which should generally mean the party leader plays the player dialogue lines and is OwningController. 
	 */
	if (GetNetMode() == NM_DedicatedServer)
	{
		if (UTalesComponent* LeaderComp = GetPartyLeader())
		{
			return LeaderComp->GetOwningController();
		}
	}
