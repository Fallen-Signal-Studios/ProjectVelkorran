	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (UDialogueBlueprintGeneratedClass* BGClass = Cast<UDialogueBlueprintGeneratedClass>(GetClass()))
		{
			...
			if (StartDialogue)
			{
				...
				if (PlayParams.bOverride_bCanBeExited)
				{
					bCanBeExited = PlayParams.bCanBeExited;
				}
...
{	
	...
	{
		if (IsValid(Speaker.NPCDataAsset))
		{
			Speaker.SpeakerID = Speaker.NPCDataAsset->NPCID;
		}
		else
		{
			Speaker.SpeakerID = FName();
		}
	}

	if (PlayerSpeakerInfo.GetSpeakerID() != NAME_PlayerSpeakerID)
	{
...
void UDialogue::OnBeginDialogue()
{
	...
	FString RoleStr = OwningComp->GetOwnerRole() >= ROLE_Authority ? "Server" : "Client";
	UE_LOG(LogTemp, Warning, TEXT("%s OnBeginDialogue %s"), *RoleStr, *GetNameSafe(this));
	...
	TArray<FSpeakerInfo> SpeakersToIter = Speakers;

	SpeakersToIter.Add(PlayerSpeakerInfo);

	for (auto& Speaker : SpeakersToIter)
	{
		if (SpeakerAvatars.Contains(Speaker.GetSpeakerID()))
		{
			if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(SpeakerAvatars[Speaker.GetSpeakerID()]))
			{
				NChar->OnEnterDialogue(this);

				if (OwningComp->HasAuthority())
				{
					if (UAbilitySystemComponent* ASC = NChar->GetAbilitySystemComponent())
					{
						UE_LOG(LogTemp, Warning, TEXT("%s Added speaker %s owned tags"), *RoleStr, *Speaker.GetSpeakerID().ToString());
						ASC->AddLooseGameplayTags(Speaker.OwnedTags, 1, EGameplayTagReplicationState::CountToOwner);
					}
				}
			}
		}
	...
	//Party members dont currently have their own speaker infos, just apply player speaker info to them should be fine 
	if (UNarrativePartyComponent* Party = Cast<UNarrativePartyComponent>(OwningComp))
	{
		...
		{
			if (ANarrativePlayerState* PState = Cast<ANarrativePlayerState>(PartyMember))
			{
				if (OwningComp->HasAuthority())
				{
					if (UAbilitySystemComponent* ASC = PState->GetAbilitySystemComponent())
					{
						UE_LOG(LogTemp, Warning, TEXT("%s Added party speaker %s owned tags"), *RoleStr, *PState->GetPlayerName());
						ASC->AddLooseGameplayTags(PlayerSpeakerInfo.OwnedTags, 1, EGameplayTagReplicationState::CountToOwner);
					}
				}
			}
		}
	...
	if (WantsCinematicBars())
	{
		if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(OwningPawn))
		{
			NChar->GetAbilitySystemComponent()->AddLooseGameplayTag(FNarrativeGameplayTags::Get().State_Player_WantsCinematicBars);
		}
	}
...
void UDialogue::OnEndDialogue()
{
	...
	FString RoleStr = OwningComp->GetOwnerRole() >= ROLE_Authority ? "Server" : "Client";
	UE_LOG(LogTemp, Warning, TEXT("%s OnEndDialogue %s"), *RoleStr, *GetNameSafe(this));
		...
		if (SpeakerAvatars.Contains(Speaker.GetSpeakerID()))
		{
			if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(SpeakerAvatars[Speaker.GetSpeakerID()]))
			{
				...
				if (OwningComp->HasAuthority())
				{
					if (UAbilitySystemComponent* ASC = NChar->GetAbilitySystemComponent())
					{
						UE_LOG(LogTemp, Warning, TEXT("%s Removed speaker %s owned tags"), *RoleStr, *Speaker.GetSpeakerID().ToString());

						ASC->RemoveLooseGameplayTags(Speaker.OwnedTags, 1, EGameplayTagReplicationState::CountToOwner);
					}
				}
			}
		}
		...
		for (APlayerState* PartyMember : Party->GetPartyMemberStates())
		{
			if (ANarrativePlayerState* PState = Cast<ANarrativePlayerState>(PartyMember))
			{
				if (OwningComp->HasAuthority())
				{
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("%s Added party speaker %s owned tags"), *RoleStr, *PState->GetPlayerName());
						ASC->RemoveLooseGameplayTags(PlayerSpeakerInfo.OwnedTags, 1, EGameplayTagReplicationState::CountToOwner);
					}
				}
			}
		}
	...
	if (WantsCinematicBars())
	{
		if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(OwningPawn))
		{
			NChar->GetAbilitySystemComponent()->RemoveLooseGameplayTag(FNarrativeGameplayTags::Get().State_Player_WantsCinematicBars);
		}
	}
...
void UDialogue::InitSpeakerAvatars()
{
	...
	for (auto& Speaker : Speakers)
	{
		if (AActor* SpeakerActor = LinkSpeakerAvatar(Speaker, Idx))
		{
			...
			{			
				...
				Speaker.SpeakerAvatar = SpeakerActor;
			}
	...
	if (AActor* SpeakerActor = OwningPawn)
	{
		...
		PlayerSpeakerInfo.SpeakerAvatar = SpeakerActor;
...
{
	//For parties/networked , whatever player def was selected use that player
	if (UDialogueNode_Player* PNode =  Cast<UDialogueNode_Player>(CurrentNode))
	{
		if (PNode->PlayerOverride)
		{
			if (UNarrativeCharacterSubsystem* NPCS = GetWorld()->GetSubsystem<UNarrativeCharacterSubsystem>())
			{
				return NPCS->FindCharacterByID(PNode->PlayerOverride->CharacterID);
			}
		}
	}
...
FVector UDialogue::GetSpeakerHeadLocation_Implementation(class AActor* Actor) const
{
	...
	{	
		if (USkeletalMeshComponent* SkelMesh = Cast<USkeletalMeshComponent>(ActorSkelMesh))
		{
			...
			{
				if (SkelMesh->DoesSocketExist(DefaultHeadBoneName))
				{
					EyesLoc.Z = SkelMesh->GetSocketLocation(DefaultHeadBoneName).Z;
				}
				else
				{
	...
	if (UDialogueNode_Player* PlayerNode = Cast<UDialogueNode_Player>(CurrentNode))
	{
		...
		//No need, generate dialogue chunk already did this: if (PlayerNode->AreConditionsMet(OwningPawn, OwningController, OwningComp))
		{
			...
			if (OwningComp && OwningComp->HasAuthority())
			{
				...
				//TODO this doesn't take into account sorting the condition checks 
...
AActor* UDialogue::LinkSpeakerAvatar_Implementation(const FSpeakerInfo& Info, const int32 Idx)
{
	...
	//If playparams set a speaker for this idx, use that 
	if (PlayParams.Speakers.IsValidIndex(Idx))
	{
		return PlayParams.Speakers[Idx];
	}

	// Find the speaker using their NPC asset. 
	if (IsValid(Info.NPCDataAsset))
	{
		//This only works for unique NPCs - non uniques need a way to specify who wants to be the speaker 
		if (UNarrativeCharacterSubsystem* NPCS = GetWorld()->GetSubsystem<UNarrativeCharacterSubsystem>())
		{
			return NPCS->FindNPC(Info.NPCDataAsset);
		}
	}
...
void UDialogue::PlayDialogueAnimation_Implementation(class UDialogueNode* Node, const FDialogueLine& Line, class AActor* Speaker, class AActor* Listener)
{
	...
	{ 
		...
		{
			if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(Speaker))
			{
				if (ANarrativeCharacterVisual* CharVis = NChar->GetCharacterVisual())
				{
					if (USkeletalMeshComponent* FaceMesh = CharVis->GetFaceMesh())
					{
...
void UDialogue::AdjustPlayerTransform_Implementation()
{
	...
	if (Speakers.Num() == 1 && Speakers.IsValidIndex(0))
	{
		if (OwningPawn && OwningController)
		{
			if (UNarrativeCharacterSubsystem* NPCS = GetWorld()->GetSubsystem<UNarrativeCharacterSubsystem>())
			{
				if (const ANarrativeCharacter* You = Cast<ANarrativeCharacter>(OwningPawn))
				{
					if (const ANarrativeNPCCharacter* Them = NPCS->FindNPC(Speakers[0].NPCDataAsset))
					{
						const FTransform PlayerDesiredTransform = PlayerAutoAdjustTransform * Them->GetActorTransform();

						//auto MoveTask = UGameplayTask_MoveToLocationAndRotation::MoveToLocationAndRotation(You->GetAbilitySystemComponent(), PlayerDesiredTransform.GetLocation(), PlayerDesiredTransform.GetRotation().Rotator(), 50.f);
						//MoveTask->ReadyForActivation(); // Begins execution of the task
						//
