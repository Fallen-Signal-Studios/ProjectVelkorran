...
void UNarrativeDialogueSequence::BeginPlaySequence(class ALevelSequenceActor* InSequenceActor, class UDialogue* InDialogue, class AActor* InSpeaker, class AActor* InListener)
{
	if (!InSpeaker)
	{
		UE_LOG(LogTemp, Warning, TEXT("No speaker was found when trying to play sequence %s"), *GetNameSafe(this));
		return;
	}
	
	if (!InDialogue)
	{
		UE_LOG(LogTemp, Warning, TEXT("No dialogue was found when trying to play sequence %s"), *GetNameSafe(this));
		return;
	}
