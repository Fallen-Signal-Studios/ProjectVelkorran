...
UTalesComponent::UTalesComponent()
{
	...
	OnNarrativeDataTaskCompleted.AddDynamic(this, &UTalesComponent::NarrativeDataTaskCompleted);
	OnQuestStarted.AddDynamic(this, &UTalesComponent::QuestStarted);
	OnQuestFailed.AddDynamic(this, &UTalesComponent::QuestFailed);
	OnQuestSucceeded.AddDynamic(this, &UTalesComponent::QuestSucceeded);
	OnQuestForgotten.AddDynamic(this, &UTalesComponent::QuestForgotten);
	OnQuestBranchCompleted.AddDynamic(this, &UTalesComponent::QuestBranchCompleted);
	OnQuestNewState.AddDynamic(this, &UTalesComponent::QuestNewState);
	OnQuestTaskProgressChanged.AddDynamic(this, &UTalesComponent::QuestTaskProgressMade);

	OnDialogueBegan.AddDynamic(this, &UTalesComponent::DialogueBegan);
	OnDialogueFinished.AddDynamic(this, &UTalesComponent::DialogueFinished);

	bIsLoading = false;

	FallbackCooldown = 10.f; 
}



void UTalesComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UTalesComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UTalesComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(UTalesComponent, PendingUpdateList);
	DOREPLIFETIME(UTalesComponent, PartyComponent);
...
bool UTalesComponent::HasAuthority() const
{
	return GetOwnerRole() >= ROLE_Authority;
}

bool UTalesComponent::IsPartyComponent() const
{
	return false;
}

bool UTalesComponent::IsQuestStartedOrFinished(TSubclassOf<class UQuest> QuestClass) const
{
...
bool UTalesComponent::IsQuestInProgress(TSubclassOf<class UQuest> QuestClass) const
{  
	if (!IsValid(QuestClass))
	{
		return false;
...
bool UTalesComponent::IsQuestSucceeded(TSubclassOf<class UQuest> QuestClass) const
{
	if (!IsValid(QuestClass))
	{
		return false;
...
bool UTalesComponent::IsQuestFailed(TSubclassOf<class UQuest> QuestClass) const
{
	if (!IsValid(QuestClass))
	{
		return false;
...
bool UTalesComponent::IsQuestFinished(TSubclassOf<class UQuest> QuestClass) const
{
	if (!IsValid(QuestClass))
	{
		return false;
...
class UQuest* UTalesComponent::BeginQuest(TSubclassOf<class UQuest> QuestClass, FName StartFromID /*= NAME_None*/)
{
...
class UQuest* UTalesComponent::BeginQuest(const FNarrativeSavedQuest& SavedQuest)
{
	if (SavedQuest.QuestClass == UQuest::StaticClass())
	{
		UE_LOG(LogNarrative, Warning, TEXT("BeginQuest was passed UQuest. Supplied quest must be a child of UQuest. "));
		return nullptr;
	}

	if (UQuest* NewQuest = MakeQuestInstance(SavedQuest.QuestClass))
	{
		//This version loads serialized data back in prior to the beginquest call so all data is good to go on load. 
		FMemoryReader MemReader(SavedQuest.QuestByteData);
		FObjectAndNameAsStringProxyArchive Ar(MemReader, true);
		Ar.ArIsSaveGame = true;

		NewQuest->Serialize(Ar);

		//If loading from save file don't send update since server will batch all quests and send them to client to begin 
		//TODO this probably won't be able to restore SaveGame vars unless we also send their bytedata across network, which seems not ideal 
		if (!bIsLoading && HasAuthority() && GetNetMode() != NM_Standalone)
		{
			SendNarrativeUpdate(FNarrativeUpdate::BeginQuest(SavedQuest.QuestClass, SavedQuest.CurrentStateID));
		}

		/*Call this after SendNarrativeUpdate as BeginQuest itself may trigger another SendNarrativeUpdateand we want BeginQuest to get called
		before any other narrative updates come through*/
		NewQuest->BeginQuest(SavedQuest.CurrentStateID);
...
bool UTalesComponent::RestartQuest(TSubclassOf<class UQuest> QuestClass, FName StartFromID)
{
	if (!IsValid(QuestClass))
	{
		return false;
...
bool UTalesComponent::ForgetQuest(TSubclassOf<class UQuest> QuestClass)
{
	if (!IsValid(QuestClass))
	{
		return false;
...
bool UTalesComponent::HasDialogueAvailable(TSubclassOf<class UDialogue> DialogueClass, const FDialoguePlayParams PlayParams)
{
...
bool UTalesComponent::BeginDialogue(TSubclassOf<class UDialogue> DialogueClass, const FDialoguePlayParams PlayParams)
{
	if (HasAuthority())
	{
...
void UTalesComponent::ClientBeginDialogue_Implementation(TSubclassOf<class UDialogue> DialogueClass, const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
	if (IsValid(DialogueClass))
	{
...
void UTalesComponent::BeginPartyDialogue(TSubclassOf<class UDialogue> Dialogue, const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
	if (HasAuthority() && PartyComponent)
	{
		//Server needs to clear any local dialogue now also.
		ExitDialogue(EExitDialogueReason::EDR_NewDialogueStarted);
...
void UTalesComponent::ClientBeginPartyDialogue_Implementation(TSubclassOf<class UDialogue> Dialogue, const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
...
void UTalesComponent::ClientExitPartyDialogue_Implementation(const EExitDialogueReason Reason)
{
	if (PartyComponent)
	{
...
void UTalesComponent::ClientRecieveDialogueChunk_Implementation(const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
	if (!HasAuthority())
	{
		if (CurrentDialogue)
...
bool UTalesComponent::TryExitDialogue(const EExitDialogueReason Reason)
{
...
void UTalesComponent::ServerTryExitDialogue_Implementation(const EExitDialogueReason Reason)
{
...
void UTalesComponent::SelectDialogueOption(class UDialogueNode_Player* Option, class APlayerState* Selector)
{
...
void UTalesComponent::ClientSelectDialogueOption_Implementation(const FName& OptionID, class APlayerState* Selector)
{
	if (CurrentDialogue)
	{
...
void UTalesComponent::ServerSelectDialogueOption_Implementation(const FName& OptionID)
{
...
bool UTalesComponent::TrySkipCurrentDialogueLine()
{
...
bool UTalesComponent::SkipCurrentDialogueLine()
{
...
void UTalesComponent::ServerTrySkipCurrentDialogueLine_Implementation()
{
	TrySkipCurrentDialogueLine();
}

bool UTalesComponent::CompleteNarrativeDataTask(const UNarrativeDataTask* Task, const FString& Argument, const int32 Quantity)
{
	/**
...
bool UTalesComponent::CompleteNarrativeDataTask(const FString& TaskName, const FString& Argument, const int32 Quantity)
{
	if (HasAuthority())
	{
...
class UQuest* UTalesComponent::MakeQuestInstance(TSubclassOf<class UQuest> QuestClass)
{
	if (IsValid(QuestClass))
	{
...
bool UTalesComponent::CompleteNarrativeTask_Internal(const FString& RawTaskString, const bool bFromReplication, const int32 Quantity)
{
...
class UDialogue* UTalesComponent::MakeDialogueInstance(TSubclassOf<class UDialogue> DialogueClass, const FDialoguePlayParams PlayParams)
{
	if (IsValid(DialogueClass))
	{
...
bool UTalesComponent::HasCompletedTask(const UNarrativeDataTask* Task, const FString& Name, const int32 Quantity /*= 1 */)
{
	if (!Task)
	{
		return false;
...
int32 UTalesComponent::GetNumberOfTimesTaskWasCompleted(const UNarrativeDataTask* Task, const FString& Name)
{
	if (!Task)
	{
		return 0;
...
void UTalesComponent::PrepareForSave_Implementation()
{
	//Populate the saved quests array, we're about to serialize it to disk 
	SavedQuests.Empty();
	...
	for (auto& Quest : QuestList)
	{
		if (Quest)
		{
			...
			//Serialize the quests savegame vars. 
			FMemoryWriter MemWriter(Save.QuestByteData);
			FObjectAndNameAsStringProxyArchive Ar(MemWriter, true);
			Ar.ArIsSaveGame = true;

			Quest->Serialize(Ar);

			SavedQuests.Add(Save);
		}
	}
}
...
void UTalesComponent::PerformLoad()
{
	...
	for (auto& SaveQuest : SavedQuests)
	{
		...
		if (UQuest* BegunQuest = BeginQuest(SaveQuest))
		{
...
void UTalesComponent::SendNarrativeUpdate(const FNarrativeUpdate& Update)
{
	if (HasAuthority() && GetNetMode() != NM_Standalone)
	{
...
void UTalesComponent::OnRep_PartyComponent(class UNarrativePartyComponent* OldPartyComponent)
{
	if (PartyComponent)
	{
...
void UTalesComponent::OnRep_PendingUpdateList()
{
...
bool UTalesComponent::IsQuestValid(const UQuest* Quest, FString& OutError)
{
...
void UTalesComponent::NarrativeDataTaskCompleted(const UNarrativeDataTask* NarrativeTask, const FString& Name)
{
...
void UTalesComponent::QuestStarted(const UQuest* Quest)
{
	if (Quest)
	{
...
void UTalesComponent::QuestForgotten(const UQuest* Quest)
{
	if (Quest)
	{
...
void UTalesComponent::QuestFailed(const UQuest* Quest, const FText& QuestFailedMessage)
{
	if (Quest)
	{
...
void UTalesComponent::QuestSucceeded(const UQuest* Quest, const FText& QuestSucceededMessage)
{
...
void UTalesComponent::QuestNewState(UQuest* Quest, const UQuestState* NewState)
{
...
void UTalesComponent::QuestTaskProgressMade(const UQuest* Quest, const UNarrativeTask* Task, const class UQuestBranch* Branch, int32 OldProgress, int32 NewProgress)
{
	if (Quest && Task)
	{
...
void UTalesComponent::QuestTaskCompleted(const UQuest* Quest, const UNarrativeTask* Task, const class UQuestBranch* Branch)
{

}

void UTalesComponent::QuestBranchCompleted(const UQuest* Quest, const class UQuestBranch* Branch)
{

}

void UTalesComponent::DialogueRepliesAvailable(class UDialogue* Dialogue, const TArray<UDialogueNode_Player*>& PlayerReplies)
{

}

void UTalesComponent::DialogueLineStarted(class UDialogue* Dialogue, UDialogueNode* Node, const FDialogueLine& DialogueLine)
{

}

void UTalesComponent::DialogueLineFinished(class UDialogue* Dialogue, UDialogueNode* Node, const FDialogueLine& DialogueLine)
{

}

void UTalesComponent::DialogueBegan(UDialogue* Dialogue)
{
	
}

void UTalesComponent::DialogueFinished(UDialogue* Dialogue, const bool bStartingNewDialogue, const EExitDialogueReason Reason)
{

}

APawn* UTalesComponent::GetOwningPawn() const
{
	if (OwnerPC)
	{
...
APlayerController* UTalesComponent::GetOwningController() const
{
...
class UDialogue* UTalesComponent::GetCurrentDialogue() const
{
...
TArray<UQuest*> UTalesComponent::GetFailedQuests() const
{
...
TArray<UQuest*> UTalesComponent::GetSucceededQuests() const
{
...
TArray<UQuest*> UTalesComponent::GetInProgressQuests() const
{
...
TArray<UQuest*> UTalesComponent::GetAllQuests() const
{
	return QuestList;
}

class UQuest* UTalesComponent::GetQuestInstance(TSubclassOf<class UQuest> QuestClass) const
{
	for (auto& QIP : QuestList)
	{
...
void UTalesComponent::ClientReceiveSave_Implementation(const TArray<FNarrativeSavedQuest>& InSavedQuests, const TArray<FString>& Tasks, const TArray<int32>& Quantities)
{
	...
	MasterTaskList = RemadeTaskList;
	SavedQuests = InSavedQuests;
