...
for b in fit['baseline']:
    ...
    if '--only' not in sys.argv or sys.argv[sys.argv.index('--only')+1]==b['asset_prefix']:
        runpy.run_path(str(root/'build_approach_bridge_kit.py'),init_globals=dict(KIT_ROOT=str(folder),SPAN_LENGTH=b['length_m'],MESH_PREFIX=b['asset_prefix'],EAST_NORTH_OPENING=b.get('east_north_opening_m',0)))
