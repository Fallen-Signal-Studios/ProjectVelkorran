...
def path(name,points,width,depth,mat):
    ...
    for i in range(len(points)-1):
        a=2*i; faces += [(a,a+2,a+3,a+1),(a+count+1,a+count+3,a+count+2,a+count),
            (a,a+count,a+count+2,a+2),(a+1,a+3,a+count+3,a+count+1)]
...
def export(name,dimensions):
    ...
    manifest.append(dict(asset=name,triangles=len(o.data.loop_triangles),nominal_dimensions_m=dimensions,
        ...
        collision='Not authored; no gameplay placement until collision is verified'))
...
for x in (-1.92,1.92):
    box('Outer seam shadow',(x,-.27,3.3),(.14,.045,6.3),dark)
    box('Stone arris',(x,-.32,3.3),(.075,.11,6.3),stone)
for z,w,d,h in ((.08,4,.7,.16),(.22,3.94,.62,.1),(6.38,3.96,.62,.12),(6.53,4,.78,.18),(6.69,3.98,.72,.12),(6.86,4,.85,.22)):
...
wall.location.x=-2; duplicate=wall.copy(); duplicate.data=wall.data; scene.collection.objects.link(duplicate); duplicate.location.x=2
for x in (-4,0,4):
