"""Fresh Blender FBX round trip for the exterior scenic Z12 kit."""
...
assert manifest['dock_envelope_m'] == [28, 18]
rows = []
for spec in manifest['modules']:
    ...
    assert min(v.co.z for v in mesh.data.vertices) >= -.01 if 'Spire' in spec['asset'] else True
    assert all(math.isfinite(v.co[i]) for v in mesh.data.vertices for i in range(3))
    bad = [(p.index, p.area) for p in mesh.data.polygons if p.area <= 1e-8]
    assert not bad, (spec['asset'], bad[:10], len(bad))
    assert spec['convex_hulls'] == 0
    rows.append(dict(asset=spec['asset'], triangles=spec['triangles'],
                     dimensions_m=[round(v, 5) for v in mesh.dimensions], uv_channels=2,
                     materials=spec['materials'], bytes=path.stat().st_size))
assert len(rows) == 3
(root / 'verification.json').write_text(json.dumps(dict(status='round_trip_pass', modules=rows), indent=2))
print('Z12_OPEN_VISTA_ROUNDTRIP_PASS')
