"""Capture-gallery ceiling: deep octagonal stone coffers, unit-scale fitted edge bays."""
...
ROOT=Path(__file__).resolve().parent/'Z07CeilingKit';ROOT.mkdir(exist_ok=True)
...
def coffer(width,length,name):
    # The full backing closes every bay; articulated stone sits below it.
    box('Continuous stone roof backing',(0,0,.52),(width,length,.06),stone,.006)
    for x in (-width/2+.09,width/2-.09):
        for y in (-length/4,length/4):box('Jointed primary ceiling beam',(x,y,.11),(.18,length/2-.02,.22),stone,.014)
        box('Beam recessed fascia',(x,0,.028),(.09,length-.04,.048),dark,.005)
    for y in (-length/2+.09,length/2-.09):
        for x in (-width/4,width/4):box('Jointed transverse beam',(x,y,.11),(width/2-.012,.18,.22),stone,.014)
    rx=width/2-.22;ry=length/2-.22
    for j in range(8):
    ...
    for sx in (-1,1):
        for sy in (-1,1):
            x=sx*(width/2-.34);y=sy*(length/2-.34)
    ...
    o=export(name,[width,length,.55]);manifest[-1]['nominal_dimensions_m']=list(o.dimensions)
    manifest[-1]['collision']='None; ceiling relief stays above 5.45 m room clearance; no gameplay collision changed'
    manifest[-1]['minimum_z_m']=min(v.co.z for v in o.data.vertices)
    manifest[-1]['maximum_z_m']=max(v.co.z for v in o.data.vertices)
    o.hide_render=True
    return o

wide=coffer(34/9,4.4,'SM_Aurelion_KIT_Z07Coffer')
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'Aurelion-Z07-Ceiling.blend'))
bpy.ops.render.render(write_still=True)
print('Z07_CEILING_BUILD_PASS')
