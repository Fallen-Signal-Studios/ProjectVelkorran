...
groups={k:[] for k in ['native','kit','art','other']};inventory=[]
for actor in actors:
    ...
    for c in actor.get_components_by_class(unreal.StaticMeshComponent):
        ...
        key='native' if label.startswith('Aurelion_') else 'kit' if label.startswith('KIT_') else 'art' if label.startswith(('ART_','Z0','Z1')) else 'other'
        groups[key].append(c)
