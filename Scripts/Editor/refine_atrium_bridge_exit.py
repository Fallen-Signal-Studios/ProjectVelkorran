"""Reimport owned final bridge deck with a fitted eastbound corridor opening."""
...
exit_checks=runpy.run_path(str(root/'Scripts/Editor/check_atrium_bridge_exit.py'))
exit_before=exit_checks['check_exit'](world,False)
assert all(row['blocker']==('A_Connectors_Tarrik_to_Atrium_Guard_1_-1' if row['edge_control'] else 'KIT_North_Bridge_bridge5_NorthSpanBDeck') for row in exit_before),exit_before
changed=[]
for kit in ('NorthBridgeKit',):
    source=root/'Art/Source/Aurelion'/kit
    for spec in json.loads((source/'manifest.json').read_text())['modules']:
        if spec['asset']!='SM_Aurelion_KIT_NorthSpanBDeck':continue
...
assert len(changed)==1 and helpers['snapshot_actor_state'](actors)==before
...
exit_after=exit_checks['check_exit'](world,True)
(out/'atrium-exit-before-after.json').write_text(json.dumps(dict(before=exit_before,after=exit_after),indent=2))
map_saved=False
...
(out/'atrium-bridge-exit-refinement.json').write_text(json.dumps(dict(status='assets_saved',map_saved=map_saved,assets=changed,actor_count=len(actors),qualification='Owned mesh/material refinement; full visual and gameplay acceptance pending.'),indent=2))
...
capture=capture.replace("('entry',unreal.Vector(p.x,p.y,p.z+165),unreal.Rotator(yaw=90),90)","('exit-comparison',unreal.Vector(-6500,-2000,175),unreal.Rotator(yaw=0),90)").replace("('west-wall',unreal.Vector(-7600,-14900,165),unreal.Rotator(pitch=12,yaw=180),75)","('exit-overview',unreal.Vector(-6000,-2700,850),unreal.Rotator(pitch=-40,yaw=145),85)").replace('z01-','atrium-')
