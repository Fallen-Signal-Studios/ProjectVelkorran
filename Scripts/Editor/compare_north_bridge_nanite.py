"""Unsaved bridge comparison: Nanite versus traditional full geometry."""
import json,os,time
...
names=['ApproachDeck','ApproachArchSupports','NorthSpanADeck','NorthSpanAArchSupports','NorthSpanBDeck','NorthSpanBArchSupports']
...
camera=subsystem.spawn_actor_from_class(unreal.CameraActor,unreal.Vector(-2500,-10000,-700),unreal.Rotator(pitch=-14,yaw=132))
camera.get_component_by_class(unreal.CameraComponent).set_field_of_view(80);editor.editor_set_game_view(True);editor.pilot_level_actor(camera)
...
def tick(delta):
    ...
    try:
        ...
        if state['phase']==0 and elapsed>25:
            state['task']=unreal.AutomationLibrary.take_high_res_screenshot(1600,900,str(out/'nanite.png'),camera);state['phase']=1
        elif state['phase']==1:
            for mesh,old in zip(meshes,previous):
                settings=sm.get_nanite_settings(mesh);settings.set_editor_property('enabled',False);settings.set_editor_property('fallback_target',unreal.NaniteFallbackTarget.PERCENT_TRIANGLES);settings.set_editor_property('fallback_percent_triangles',1.0);settings.set_editor_property('fallback_relative_error',0.0)
            ...
            state['task']=unreal.AutomationLibrary.take_high_res_screenshot(1600,900,str(out/'traditional-full-geometry.png'),camera);state['phase']=3
        elif state['phase']==3:
            restore()
            assert all(sm.get_nanite_settings(m).export_text()==old.export_text() for m,old in zip(meshes,previous))
            assert (out/'nanite.png').exists() and (out/'traditional-full-geometry.png').exists()
