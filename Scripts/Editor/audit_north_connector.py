...
regions=[((-8200,-8200,-6000),(-5800,-1800,1500))]
...
probes=[]
for y in (-8000,-7400,-6800,-6200,-5600,-5000,-4400,-3800,-3200,-2600,-2000):
 for z in (75,175,275):
...
(out/'north-connector.json').write_text(json.dumps(dict(scope='Authoring survey including instance bounds, not just instance origins; global enclosures may intersect the region. No live route qualification.',bridges=bridges,components=rows,probes=probes),indent=2))
assert not unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()
editor.editor_set_game_view(True)
capture=(root/'Scripts/Editor/fit_z01_lower_architecture.py').read_text(encoding='utf-8-sig').split("p=by_label['Z01_Entry_StandIn']",1)[1]
capture=capture.replace("('entry',unreal.Vector(p.x,p.y,p.z+165),unreal.Rotator(yaw=90),90)","('north-connector',unreal.Vector(-7000,-7900,175),unreal.Rotator(yaw=90),90)").replace("('west-wall',unreal.Vector(-7600,-14900,165),unreal.Rotator(pitch=12,yaw=180),75)","('connector-overview',unreal.Vector(-4200,-6600,2300),unreal.Rotator(pitch=-30,yaw=145),90)").replace('z01-','north-')
