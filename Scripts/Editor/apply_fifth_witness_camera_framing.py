"""Save only FifthWitness camera tracks, retaining the authored character performances."""
...
map_file=project/'Content/Aurelion/Maps/L_Aurelion_M13.umap'
...
sequence=unreal.load_asset('/Game/Aurelion/Cinematics/LS_FifthWitness')
...
for name,key in [('CameraWide','wide'),('CameraClose','close')]:
    position,target,focal=shots[key]
    camera=next(a for a in actors if isinstance(a,unreal.CineCameraActor) and 'LS_FifthWitness_'+name in a.get_actor_label())
    assert abs(camera.get_cine_camera_component().current_focal_length-focal)<.01,'Focal mismatch: this tool edits sequence tracks only'
