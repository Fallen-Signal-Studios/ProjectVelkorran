...
def tick(delta):
    ...
    try:
        ...
        if state['phase']==0 and elapsed>25:
            state['task']=unreal.AutomationLibrary.take_high_res_screenshot(1600,900,str(out/'default-fallback.png'),camera);state['phase']=1
        elif state['phase']==1:
            for mesh,old in zip(meshes,previous):
                settings=sm.get_nanite_settings(mesh);settings.set_editor_property('fallback_target',unreal.NaniteFallbackTarget.PERCENT_TRIANGLES);settings.set_editor_property('fallback_percent_triangles',1.0);settings.set_editor_property('fallback_relative_error',0.0)
            ...
            state['task']=unreal.AutomationLibrary.take_high_res_screenshot(1600,900,str(out/'full-fallback.png'),camera);state['phase']=3
        elif state['phase']==3:
            restore()
            assert all(sm.get_nanite_settings(m).export_text()==old.export_text() for m,old in zip(meshes,previous))
            assert (out/'default-fallback.png').exists() and (out/'full-fallback.png').exists()
