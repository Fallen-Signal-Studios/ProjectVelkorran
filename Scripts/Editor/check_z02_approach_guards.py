...
def placements():
    return [(f'{side}_{i}',(x,-11700+400*i,0),90) for side,x in [('West',-7288),('East',-6712)] for i in range(5)]

def check_guards(world,actors):
    labels={a.get_actor_label():a for a in actors}
    fit=json.loads((Path(unreal.Paths.project_dir())/'Art/Source/Aurelion/ParapetKit/guard-fit.json').read_text())
    sm=unreal.get_editor_subsystem(unreal.StaticMeshEditorSubsystem)
    for name,pos,yaw in placements():
        a=labels['KIT_Z02_ApproachGuard_South_'+name];c=a.static_mesh_component
        ...
        assert max(abs(v-w) for v,w in zip((e.x,e.y,e.z),(11,200,65)))<.1 and abs(o.z-65)<.1
        assert c.static_mesh.get_name()=='SM_Aurelion_KIT_Parapet_4m'
    ...
    for y in (-11700,-11300,-10900,-10500,-10100):
    ...
    return dict(placements=10,retained_guard_count=2,pawn_controls=probes,qualification='Isolated guard collision and visual fit; full bridge art and live traversal remain pending.')
