"""Fit the coffered roof to the original 56-instance ceiling actor."""
from pathlib import Path
import json,os,runpy,shutil,unreal
root=Path(unreal.Paths.project_dir());source=root/'Art/Source/Aurelion/Z09CeilingKit';out=Path(os.environ['SOV_AURELION_RUN_DIRECTORY'])
...
old=json.loads((source/'ceiling-baseline.json').read_text(encoding='utf-8-sig'));manifest=json.loads((source/'manifest.json').read_text())
...
assert not json.loads((source/'Z09Coffer-coplanar.json').read_text())['overlaps']
dest='/Game/Aurelion/Environment/ArchitectureKit';persist=bool(globals().get('PERSIST_Z09_CEILING',False))
...
(out/'z09-ceiling-fit.json').write_text(json.dumps(dict(status='saved' if persist else 'unsaved_preview',geometry=result),indent=2))
...
views="views=[('entry',(-300,25750,-1330),(12,90),80),('coffer',(0,27000,-1250),(65,90),80),('reverse',(350,30200,-1330),(12,-90),80)]\n"
exec(compile(prefix+views+'state=dict'+suffix,'z09_ceiling_capture','exec'),globals())
