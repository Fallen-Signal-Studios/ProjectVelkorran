...
(out/'z02-roof-architecture.json').write_text(json.dumps(dict(scope='Z02 plus approach inventory only; no save or gameplay acceptance',bounds=[lo,hi],components=rows),indent=2))
assert not unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()
editor.editor_set_game_view(True)
root=Path(unreal.Paths.project_dir())
capture=(root/'Scripts/Editor/fit_z01_lower_architecture.py').read_text(encoding='utf-8-sig').split("p=by_label['Z01_Entry_StandIn']",1)[1]
capture=capture.replace("('entry',unreal.Vector(p.x,p.y,p.z+165),unreal.Rotator(yaw=90),90)","('roof-approach',unreal.Vector(-7000,-11200,1300),unreal.Rotator(pitch=-10,yaw=90),90)").replace("('west-wall',unreal.Vector(-7600,-14900,165),unreal.Rotator(pitch=12,yaw=180),75)","('roof-overview',unreal.Vector(-4900,-11100,2600),unreal.Rotator(pitch=-30,yaw=135),85)").replace('z01-','z02-')
exec(compile("p=by_label['Z02_Entry_StandIn']"+capture,'z02_roof_audit_capture','exec'))
