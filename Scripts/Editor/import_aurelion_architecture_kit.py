...
for spec in json.loads((source/'manifest.json').read_text())['modules']:
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(path):
        previous=unreal.load_asset(path)
        assert Path(previous.get_editor_property('asset_import_data').get_first_filename()).resolve()==(source/(name+'.fbx')).resolve()
    ...
    if spec.get('convex_hulls',0):
        assert sm.get_convex_collision_count(mesh)==spec['convex_hulls']
    elif 'Cornice' not in name and sm.get_simple_collision_count(mesh)==0:
    ...
    rows.append(dict(asset=path,dimensions_cm=dimensions,materials=slots,
        simple_collision=sm.get_simple_collision_count(mesh),convex_collision=sm.get_convex_collision_count(mesh),nanite=True,lightmap_channel=1))

review='/Game/Aurelion/ArtReview/L_Aurelion_ArchitectureKit'
if unreal.EditorAssetLibrary.does_asset_exist(review):
    (out/'kit-import.json').write_text(json.dumps(dict(status='refreshed_assets',meshes=rows,map=review,limitations='Review placements unchanged'),indent=2))
else:
