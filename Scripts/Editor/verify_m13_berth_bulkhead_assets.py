"""Fresh UE asset-load check for rejected-map-placement Z12 source candidates."""
import hashlib
import json
import os
...
(out / 'berth-bulkhead-fresh-assets.json').write_text(json.dumps(dict(
    status='fresh_assets_pass', assets=rows, map_hashes=maps,
    qualification='Fresh editor asset load only; full rear-wall placement was rejected.'), indent=2))
print('Z12_BERTH_BULKHEAD_FRESH_ASSETS_PASS')
