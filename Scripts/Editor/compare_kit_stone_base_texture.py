"""Unsaved base-color-only isolation; preserve the stone normal and roughness inputs."""
import json,os
...
base=lib.get_material_property_input_node(material,unreal.MaterialProperty.MP_BASE_COLOR);assert isinstance(base,unreal.MaterialExpressionLinearInterpolate)
inputs=lib.get_inputs_for_material_expression(material,base);texture_node=lib.get_inputs_for_material_expression(material,inputs[1])[0];texture=texture_node.get_editor_property('texture')
...
(out/'base-texture-isolation.json').write_text(json.dumps(dict(status='unsaved_diagnostic',texture=texture.get_path_name(),source_filename=source,before_base_texture_blend=before,after_base_texture_blend=0,normal_and_roughness_unchanged=True,scope='PavingIvory is shared; this temporary change affects every use in the preview, and is not saved.'),indent=2))
