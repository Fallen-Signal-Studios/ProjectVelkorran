...
groups={k:[] for k in ['vault','recess','baffle','zone']};inventory=[]
for actor in actors:
    ...
    for c in actor.get_components_by_class(unreal.StaticMeshComponent):
        ...
        if not label.startswith(('ART_','Z0','Z1')):continue
        key='vault' if label=='ART_Crucible_UpperVault' else 'recess' if label.startswith('ART_RecessPanel') else 'baffle' if label.startswith('ART_RefugeBaffle') else 'zone'
        groups[key].append(c)
        inventory.append(dict(group=key,actor=label,hidden_in_game=c.get_editor_property('hidden_in_game'),component=c.get_path_name(),mesh=c.static_mesh.get_path_name(),materials=[c.get_material(i).get_path_name() if c.get_material(i) else None for i in range(c.get_num_materials())]))
