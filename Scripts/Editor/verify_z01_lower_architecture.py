...
z06_end_count = sum(a.get_actor_label().startswith('KIT_Z06_End_') for a in actors)
assert z06_end_count in (0, 18), 'Partial or duplicate Z06 end walls'
z06_slab_count = sum(a.get_actor_label()=='KIT_Z06_FallenMasonry' for a in actors)
assert z06_slab_count in (0, 1), 'Duplicate Z06 fallen masonry'
z06_refuge_count = sum(a.get_actor_label().startswith('KIT_Z06_Refuge_') for a in actors)
...
z06_flank_landing_count = sum(a.get_actor_label()=='KIT_Z06_FlankLanding' for a in actors)
assert z06_flank_landing_count in (0,1), 'Duplicate flank landing'
z07_paving_count = sum(a.get_actor_label().startswith('KIT_Z07_Paving_') for a in actors)
assert z07_paving_count in (0,54), 'Partial or duplicate Z07 paving'
z07_wall_count = sum(a.get_actor_label().startswith('KIT_Z07_Wall_') for a in actors)
