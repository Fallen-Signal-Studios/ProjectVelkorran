"""Mount three synchronized Eclipse fissures, replacing decorative floating cubes."""
from pathlib import Path
import json,os,runpy,shutil,unreal
root=Path(unreal.Paths.project_dir());source=root/'Art/Source/Aurelion/Z09SlitKit';out=Path(os.environ['SOV_AURELION_RUN_DIRECTORY'])
...
dest='/Game/Aurelion/Environment/ArchitectureKit';persist=bool(globals().get('PERSIST_Z09_SLITS',False))
...
for row in manifest['placements']:
    ...
    if spec['asset'] not in imported:imported[spec['asset']]=unreal.load_asset(dest+'/Meshes/'+spec['asset']) if persist else helper['import_owned_mesh'](spec,source,dest+'/Meshes',materials)
    mesh=imported[spec['asset']];assert mesh
...
result=runpy.run_path(str(root/'Scripts/Editor/check_z09_slits.py'))['check_z09_slits'](actors)
runpy.run_path(str(root/'Scripts/Editor/check_z09_floor.py'))['check_z09_floor'](actors)
if persist:
    shutil.copy2(root/'Content/Aurelion/Maps/L_Aurelion_M12.umap',out/'L_Aurelion_M12-before.umap');assert editor.save_current_level()
(out/'z09-slits-fit.json').write_text(json.dumps(dict(status='saved' if persist else 'unsaved_preview',geometry=result,preserved_other_actors=len(others)),indent=2))
...
views="views=[('east-context',(0,27600,-1330),(10,30),80),('panel-detail',(550,28000,-1212),(0,0),70),('gallery-context',(-300,27600,-1330),(15,90),80)]\n"
