"""Fit custom guardrail into the original visual-only component."""
from pathlib import Path
import json,os,runpy,shutil,unreal
root=Path(unreal.Paths.project_dir());out=Path(os.environ['SOV_AURELION_RUN_DIRECTORY']);source=root/'Art/Source/Aurelion/Z08RailingKit'
...
old=json.loads((source/'rail-baseline.json').read_text());fit=json.loads((source/'rail-fit.json').read_text());check=runpy.run_path(str(root/'Scripts/Editor/check_z08_railings.py'))
a=next(a for a in actors if a.get_actor_label()==old['actor']);c=a.get_component_by_class(unreal.InstancedStaticMeshComponent)
assert c.static_mesh.get_path_name()==old['mesh'] and c.get_instance_count()==44
assert [c.get_instance_transform(i,world_space=True).export_text() for i in range(44)]==[row['transform'] for row in old['instances']]
...
result=check['check_z08_railings'](actors);persist=bool(globals().get('PERSIST_Z08_RAILINGS',False))
if persist:
    shutil.copy2(root/'Content/Aurelion/Maps/L_Aurelion_M12.umap',out/'L_Aurelion_M12-before.umap');assert editor.save_current_level()
(out/'z08-railing-fit.json').write_text(json.dumps(dict(status='saved' if persist else 'unsaved_preview',settings=result,preserved_actor_states=len(actors)),indent=2))
exec(compile((root/'Scripts/Editor/review_z08_railings.py').read_text(),'review_railings','exec'),globals())
