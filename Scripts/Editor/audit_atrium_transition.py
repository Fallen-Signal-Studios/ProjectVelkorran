...
regions=[((-7600,-2700,-200),(-1000,2500,3500))]
...
(out/'atrium-transition.json').write_text(json.dumps(dict(scope='Authoring survey including instance bounds, not just instance origins; global enclosures may intersect the region. No live route qualification.',bridges=bridges,components=rows,probes=probes),indent=2))
assert not unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()
editor.editor_set_game_view(True)
capture=(root/'Scripts/Editor/fit_z01_lower_architecture.py').read_text(encoding='utf-8-sig').split("p=by_label['Z01_Entry_StandIn']",1)[1]
capture=capture.replace("('entry',unreal.Vector(p.x,p.y,p.z+165),unreal.Rotator(yaw=90),90)","('atrium-entry',unreal.Vector(-6500,-2000,175),unreal.Rotator(yaw=0),90)").replace("('west-wall',unreal.Vector(-7600,-14900,165),unreal.Rotator(pitch=12,yaw=180),75)","('atrium-overview',unreal.Vector(-7000,-6500,3000),unreal.Rotator(pitch=-25,yaw=45),90)").replace('z01-','atrium-')
