"""Measured paving fit and retained Z07 floor collision, excluding gameplay claims."""
import json,runpy
from pathlib import Path
import unreal

def check_z07_paving(world, actors):
    labels={a.get_actor_label():a for a in actors}
    fit=json.loads((Path(unreal.Paths.project_dir())/'Art/Source/Aurelion/Z07PavingKit/floor-fit.json').read_text())
    ...
    for ci,col in enumerate(fit['columns']):
        for ri,row in enumerate(fit['rows']):
            ...
            a=labels[f'KIT_Z07_Paving_{ci:02}_{ri:02}'];c=a.static_mesh_component;m=c.static_mesh
            p=a.get_actor_location();s=a.get_actor_scale3d();o,e=a.get_actor_bounds(False)
            assert abs(p.x-col['x'])<.01 and abs(p.y-y)<.01 and abs(o.z+e.z+900)<.01
    ...
    for row in fit['guides']:
        ...
        assert not c.get_editor_property('visible') and c.get_editor_property('hidden_in_game')
        assert not a.get_actor_enable_collision() and c.get_collision_enabled()==unreal.CollisionEnabled.NO_COLLISION
    floor=labels['Z07_Floor'];c=floor.static_mesh_component
    ...
    for x in [c['x'] for c in fit['columns']]+[-180,180]:
        for row in fit['rows']:
            y=row['y']
            raw=unreal.SystemLibrary.line_trace_single(world,unreal.Vector(x,y,-700),unreal.Vector(x,y,-1000),unreal.TraceTypeQuery.TRACE_TYPE_QUERY1,False,ignored,unreal.DrawDebugTrace.NONE,True)
            ...
            t=hit.to_tuple();assert t[9]==floor and abs(t[5].z+900)<.01,(x,y,t[5])
            probes.append(dict(x=x,y=y,z=t[5].z))
    return dict(placements=len(placements),shared_asset_collision=asset_collision,removed_floor_instances=54,retained_floor_instances=retained_floor_count,retained_centerline_instances=18,floor_queries=probes,guide_collision_change='Two 6cm-raised decorative guides retired; floor now consistently Z -900.',qualification='Stopped-editor fit and isolated collision checks; live traversal, navigation, combat and performance remain unqualified.')
