...
def check_z09_slits(actors):
    source=Path(unreal.Paths.project_dir())/'Art/Source/Aurelion/Z09SlitKit'
    ...
    for r in manifest['placements']:
        ...
        geo=runpy.run_path(str(Path(unreal.Paths.project_dir())/'Scripts/Editor/check_z08_railings.py'))
        mb=mesh.get_bounds();lo,hi=geo['bounds'](geo['corners'](c.get_world_transform(),[mb.origin.x,mb.origin.y,mb.origin.z],[mb.box_extent.x,mb.box_extent.y,mb.box_extent.z]))
        neighbors=json.loads((source.parent/'Z09FloorKit/native-floor-neighbors.json').read_text(encoding='utf-8-sig'))
        for rib in neighbors:
            if not rib['actor'].startswith('Z09_Rib_'):continue
            rlo,rhi=rib['bounds']
            assert any(hi[j]<=rlo[j] or lo[j]>=rhi[j] for j in range(3)),(r['actor'],rib['actor'])
        rows.append(r)
    return dict(status='passed',fractures=rows,actor_count=len(actors),qualification='Three visual-only east-wall panel overlays replace floating decorative cubes. Native walls remain unchanged; visual mounting and live narrative acceptance require separate review.')
