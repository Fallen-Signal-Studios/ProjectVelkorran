"""Approach guardrail coverage against every original instance envelope."""
...
def check_z09_railings(actors):
    source=Path(unreal.Paths.project_dir())/'Art/Source/Aurelion/Z09RailingKit'
    old=json.loads((source/'rail-baseline.json').read_text(encoding='utf-8-sig'));fit=json.loads((source/'rail-fit.json').read_text())
    ...
    assert a.get_path_name()==old['path'] and c.get_path_name()==old['component']
    assert a.get_actor_transform().export_text()==old['actor_transform'] and c.get_world_transform().export_text()==old['component_transform']
    ...
    assert c.get_instance_count()==4 and mesh.get_name()=='SM_Aurelion_KIT_Z09Guardrail'
    ...
    sm=unreal.get_editor_subsystem(unreal.StaticMeshEditorSubsystem);n=sm.get_nanite_settings(mesh);b=mesh.get_bounds()
    assert n.enabled and n.position_precision==10 and n.fallback_percent_triangles==1
    assert sm.get_num_uv_channels(mesh,0)==2 and sm.get_convex_collision_count(mesh)==0 and sm.get_simple_collision_count(mesh)==0
    assert {c.get_material(i).get_name() for i in range(c.get_num_materials())}=={'M_AurelionKit_PavingIvory','M_AurelionKit_Reveal','M_AurelionKit_Gold'}
    ...
    assert sorted(covered)==list(range(8)) and len(actors)==3140
    return dict(status='passed',instances=results,actor_count=len(actors),qualification='All 8 former doubled vendor envelopes covered by 4 unit-scale guardrails; visual-only component collision and native approach barriers unchanged. Live traversal and final visual acceptance remain open.')
