"""Read-only retained Crucible deck mesh and instance measurements."""
from pathlib import Path
import json,os,unreal
root=Path(unreal.Paths.project_dir());out=Path(os.environ['SOV_AURELION_RUN_DIRECTORY'])
assert not unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()
actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem).get_all_level_actors()
a=next(a for a in actors if a.get_actor_label()=='Aurelion_Art_M12_Z08_28_6cb87f')
...
(out/'deck-baseline.json').write_text(json.dumps(data,indent=2))
assert len(actors)==3140 and len(rows)==42 and c.get_collision_enabled()==unreal.CollisionEnabled.NO_COLLISION
assert not unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()
