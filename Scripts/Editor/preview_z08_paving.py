"""Fit quarantine-crucible paving while retaining its elevated surfaces and collision."""
...
assert len(original)==2935 and not any(a.get_actor_label().startswith('KIT_Z08_Paving_') for a in original)
fit=json.loads((root/'Art/Source/Aurelion/Z08PavingKit/floor-fit.json').read_text())
...
source=root/'Art/Source/Aurelion/Z08PavingKit';destination='/Game/Aurelion/Environment/ArchitectureKit'
...
for suffix in ('PavingIvory_4m',):meshes['SM_Aurelion_KIT_'+suffix]=unreal.load_asset(destination+'/Meshes/SM_Aurelion_KIT_'+suffix)
...
for ci,col in enumerate(fit['columns']):
    for ri,row in enumerate(fit['rows']):
        ...
        a=subsystem.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector(col['x'],row['y'],-1200-b.origin.z-b.box_extent.z))
        a.set_actor_label(f'KIT_Z08_Paving_{ci:02}_{ri:02}');a.set_folder_path('Aurelion/CustomArchitecture/Z08/Paving')
...
assert [c.get_instance_transform(i,world_space=True).export_text() for i in range(c.get_instance_count())]==fit['centerline']['all_instance_transforms']
c.modify()
for row in sorted(fit['selected_centerline'],key=lambda r:r['index'],reverse=True):assert c.remove_instance(row['index'])
assert helpers['snapshot_actor_state'](unchanged)==before,'Unrelated actor transform/collision changed'
geometry=runpy.run_path(str(root/'Scripts/Editor/check_z08_paving.py'))['check_z08_paving'](world,list(subsystem.get_all_level_actors()))
if persist:
    shutil.copy2(root/'Content/Aurelion/Maps/L_Aurelion_M12.umap',out/'L_Aurelion_M12-before.umap');assert editor.save_current_level()
(out/'z08-paving-fit.json').write_text(json.dumps(dict(status='saved' if persist else 'unsaved_preview',geometry=geometry,preserved_actor_states=len(unchanged)),indent=2))
if not globals().get('SKIP_PAVING_CAPTURE',False):
    editor.editor_set_game_view(True)
    capture=(root/'Scripts/Editor/fit_z01_lower_architecture.py').read_text(encoding='utf-8-sig').split("p=by_label['Z01_Entry_StandIn']",1)[1]
    capture=capture.replace("('west-wall',unreal.Vector(-7600,-14900,165),unreal.Rotator(pitch=12,yaw=180),75)","('room-return',unreal.Vector(0,22900,-940),unreal.Rotator(pitch=6,yaw=-90),90)").replace('z01-','z08-')
    exec(compile("p=by_label['Z08_Entry_StandIn']"+capture,'z08_paving_capture','exec'))
