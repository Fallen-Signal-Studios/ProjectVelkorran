...
| Resource relief | Relief opens at 25% health, when a depleted shield leaves health at or below half, or when shield and stamina are both depleted (thresholds are authored encounter data). It then persists while the player stays nearly out of resources, spacing new attacks and limiting melee slots to one, and lingers for its duration after recovery. It grants no health, stamina, shield or Echo. The cooldown counts from the moment relief lapses, so it cannot flicker. Authored challenges can disable it. |
