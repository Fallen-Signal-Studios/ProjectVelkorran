...
2. Resolve exactly one controlled protagonist and uniquely tagged real characters; require matching ASC/avatar readiness, configured damage-state tags, and any required already-owned equipped weapon with a legal hand attachment. No cinematic-only inventory copy is created.
3. Preload the declared assets and existing streaming levels with a finite timeout. Validate the sequence hierarchy, duration and exact role bindings. Reject generic Sequencer event tracks, including nested subsequences, because arbitrary event side effects cannot be replayed atomically on skip.
...
Typed postconditions cover character transforms and keeping, holstering or drawing a specified **already owned and equipped** weapon through `ANarrativeCharacter::SetWieldState`. They do not grant/remove inventory, replace equipment, change health, or replay arbitrary Blueprint events. Required inventory/reward operations must use an existing authoritative native owner and a separately validated committed beat; generic Sequencer events are rejected for managed scenes. Native postcondition verification is a finite contract, not an atomic rollback system for arbitrary gameplay callbacks.

The preload manifest supports soft assets and existing classic streaming levels. A World Partition streaming-source/cell-readiness contract is still required if authored scenes depend on unloaded partition cells; it is not implemented or implicitly guaranteed by the asset preload. Actors needed for initial participant resolution must already exist. Cross-level participant spawning, custom track side effects, per-bone pose continuity, authored camera cuts, audio/subtitle timing and replay UI require the actual assets and engine validation. Managed scenes intentionally exclude interactive choices and protagonist handoffs; those have their own native gates.
