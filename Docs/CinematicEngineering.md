...
Follow-ups: [PartitionedCinematicEngineering.md](PartitionedCinematicEngineering.md) adds native World Partition readiness and typed door/lift power/lock postconditions. [CinematicInventoryTransactions.md](CinematicInventoryTransactions.md) adds approved inventory grants/removals and equipment replacement on Narrative's existing authority, including owned rollback and weapon presentation handoff.
...
6. Both completion and skip stop/restore the same sequence, apply the same native inventory/equipment journal, typed final transforms and draw/holster state, then commit the same campaign beat. Final placements require clear capsules, navigation, existing recovery-exclusion checks and pairwise participant clearance. Entry-required weapons remain owned/equipped unless an explicit approved slot replacement declares their exit identity. Every final postcondition is checked again after all participant callbacks and again when the campaign consumes the receipt.
...
Typed postconditions cover character transforms, native draw/holster state, approved exact inventory grants/removals and equipment replacement through Narrative's existing components, and stable door/lift power/lock state through `ASovWorldTransitActor`. Inventory operations require explicit class approval, exact instance resolution, complete quantities and a retained native journal. Removed objects retain their GUID, resources and fragments; rollback never recreates their class defaults. They do not change health, move a mechanism, or replay arbitrary Blueprint events. Native postcondition verification is a finite contract, not an atomic rollback system for arbitrary gameplay callbacks.

Accepted opening cinematic requests remain pending in Loading while a real local player's first-boot accessibility setup is incomplete. Asset preloading may continue, but no playback starts and the first-boot hold does not consume the monotonic preparation timeout. Headless automation is not gated. This does not introduce an automatic opening trigger or make an invalid protagonist/mission request valid.
