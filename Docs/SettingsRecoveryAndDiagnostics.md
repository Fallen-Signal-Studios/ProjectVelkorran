...
- **Projectile lead:** the existing native releases for Hunger, Cinder Sticky Grenade and Stillpoint now use a fixed-speed, gravity-aware intercept with at most 0.6 seconds of prediction and eight degrees of correction. Current/predicted visibility, weapon range, fuse/flight duration, speed caps and a swept physical arc all constrain admission. Rejection preserves the exact original launch. Stillpoint flight now shares actual world gravity and analytic acceleration stepping with the solver. See `BallisticAssistEngineering.md` for production-math results and the authored, unexecuted UE launch/physics regressions. Arbitrary Blueprint-owned launches still require their own integration; this does not claim universal tracking.
...
| --- | --- | --- |
| Vibration intensity by channel | Native channel routing, bounded ownership, settings and actual damage/interaction/sequence producers are now implemented; see `PlatformOutputEngineering.md`. Proprietary controller output and external direct-feedback callers still require integration. | Expose channel controls, author UI/ambient producers and device-specific feedback, verify accessibility-off behavior and test supported hardware. |
| HDR calibration | Native engine capability/output queries and transactional preview/confirm/revert are implemented; see `PlatformOutputEngineering.md`. Full black-floor/paper-white/UI-luminance and external output-device identity remain open. | Supply calibration imagery and readable controls; verify actual HDR displays, monitor transitions, luminance/tone mapping and fallback behavior. |
...
| Menu focus memory/navigation wrap | CommonUI focus-cache routing, activation reentry guards and owned menu-boundary wrapping are implemented in the existing classes; see `FrontendAccessibilityEngineering.md`. Actual Slate and shipping menu acceptance remain pending. | Wire each real menu, test controller/keyboard navigation and reader focus together. |
