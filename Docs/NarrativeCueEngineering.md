...
Editor work: author localized captions/audio, cue data assets and free-movement Narrative graphs; request cues from the existing encounter/dialogue/companion events; bind `OnCueStarted` and `OnCueEnded` to subtitle/record presentation. `OnDialogueSuspensionChanged` lets the existing dialogue UI hide and restore its current choice/caption state. Complete cinematic graphs should continue through campaign cinematic assets.
