...
Required final world changes previously covered participant transforms and already-equipped weapon draw/holster state only. Add a deliberately finite native door/lift power-and-lock contract through `ASovWorldTransitActor`, whose save fields, navigation links, authority checks and notifications already exist. Inventory grants/removals and arbitrary Sequencer event replay remain unsupported; inventing a cinematic inventory owner would duplicate Narrative's authority and would not provide reliable rollback.
