...
Compiled and ran the exact production budget/ledger helper with GCC C++17 and `-Wall -Wextra -Werror -pedantic`: 3,500 checks passed, since raised to **4,575** on Apple clang with UBSan.

The zero-based rank rule — a purchased rank counts as `PerkLevel + 1` — was written out five times in the component: branch investment in `CanBuyPerk`, investment and spend in `ValidateSavedState`, and the branch-level rebuild. It is now `SovTechniquePolicy::RankWeight` in one place, with `MeetsBranchInvestment` for the tier gate, so the rule is exercised portably rather than only through a world/GAS/save stack. A failed grant sets a perk level to `-1`, so negative levels genuinely occur; `RankWeight` contributes zero for them, which also stops a level below `-1` subtracting from another perk's investment as the open-coded form would. Behaviour is otherwise unchanged, and the nine automation suites confirm it. Coverage includes the 18–22 range, claim deduplication, cap rejection, negative/overflow inputs, point conservation and free-respec arithmetic.
...
**Executed 11 September 2026.** All nine `ProjectVelkorran.Campaign.Techniques` registrations run and pass on macOS with UE 5.7 (the six above plus the three augment suites), as part of a 617-test suite. The earlier note that they "could not be executed in this environment" is superseded. Run `Scripts/Validate-UnrealMac.py --filter ProjectVelkorran.Campaign.Techniques` on macOS, or `Scripts/Validate-Unreal.ps1 -EngineRoot <UE_5.7> -TestFilter ProjectVelkorran.Campaign.Techniques` on Windows. A Windows run of these suites is still outstanding; see [MacEngineeringEnvironment-2026-09-11.md](MacEngineeringEnvironment-2026-09-11.md). Remaining editor gates are actual branch assets/point distribution, each perk's behavior and preview, first-entry/return handoffs, retry after rewards, save before/after respec, and paused/menu presentation. The TDD's full authored Technique content remains a Blueprint/data acceptance task.
