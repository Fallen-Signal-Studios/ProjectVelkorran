...
- **Ammo-sensitive replay:** that second fresh run spent almost all Cinderline rounds clearing E1, even after collecting a 63-round pickup. The E3 firearm-only validation pilot entered with one round and no reserve, then stopped when it found no reachable matching pickup. This does not establish a campaign softlock: the pilot does not exercise Tarrik's melee kit or other attacks. Test a normal player fallback and tune default ammo supply if needed; do not count the second replay as an E3 victory. The first fresh run above did earn E3 and M13.
