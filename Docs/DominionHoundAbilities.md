...
- `USovGameplayAbility_DominionHoundHornCharge`: a committed Heavy charge with continuous swept hit detection on `Narrative.Input.Ability1`.
...
Use the StateTree or Behavior Tree to choose one input tag from target distance and cooldown availability. Do not activate the generic attack input and hope Narrative resolves competing specs.

A practical first-pass policy is:

| Situation | Requested input | Expected move |
|---|---|---|
| Target inside Bite range | `Narrative.Input.Attack` | Bite |
| Target in middle distance, clear ground lane, charge ready | `Narrative.Input.Ability1` | Horn Charge |
| Target beyond Bite range, valid landing route, pounce ready | `Narrative.Input.Ability2` | Pounce |
| No move valid or ability still cooling down | None | Reposition, flank, or wait |

Before activation, check that the selected ability is granted, off cooldown, within its authored minimum/maximum range, and not blocked by death, fatal damage, Poise break, Freeze, ragdoll, interaction, sequencing, or an existing attack. Face/focus the target during wind-up. Once Horn Charge or Pounce commits, let the native ability own movement until it ends; competing AI move requests should remain postponed.

Treat distance thresholds as overlapping decision bands rather than exact rings. The overlap lets encounter logic prefer a move based on line of approach, ally spacing, player state, and recent attack history without creating dead zones. Add a small amount of decision cooldown or move-history weighting in the StateTree to avoid alternating gap closers mechanically.
