...
The Hound must be possessed by `ANarrativeNPCController` or a subclass. All three attacks use that controller's native token-lease adapter; a different controller class deliberately causes activation to fail before damage, movement, or cooldown is committed.
...
Every Hound attack reserves one of its selected target's Narrative attack tokens before it commits. At a full attacker budget, Narrative's normal token-steal rules may reassign an eligible, unreserved token during acquisition; otherwise activation is rejected. A token the Hound already held for the same target is borrowed and reserved for the ability. A Hound holding another target's token is not silently retargeted; its owning AI behavior must return that token first. While the ability is active, the reservation prevents ordinary stealing and defers a Behavior Tree `ReturnToken` request until the ability releases. A newly claimed token is returned on every success and cancellation path; a borrowed token remains with its original behavior unless that behavior requested its return during the reservation. Lease identity and reciprocal target-ASC membership are checked throughout execution, so a forced loss cancels the attack without returning a newer lease. Swept damage is restricted to the exact ASC whose slot was reserved; another hostile crossing the path cannot take unbudgeted damage.
...
Bite is the hound's dependable close-range Standard pressure tool. At its native impact time, authority sweeps from the mouth socket through the configured bite reach and applies one unified Narrative damage result only to the selected hostile whose attacker slot the ability reserved. Another body or world surface may block the bite, but another hostile cannot inherit that reservation. The per-activation hit ledger prevents the selected target from taking duplicate damage when a sweep begins or ends inside the same body.
...
| Target attacker budget | Fill the player's budget with fresh, non-stealable tokens, then free one slot; repeat for Bite, Horn Charge, and Pounce | Every move is rejected while full and becomes eligible after a slot is available; total Horn rejection consumes no Handler command cooldown |
...
| Borrowed token | Let the BT own the same target token, start each attack, and request `ReturnToken` during it | Return is deferred until the attack releases; the move never runs without a slot and the borrowed token is then returned exactly once |
| Reservation identity | Borrow the same retained token for two consecutive attacks, then invoke delayed cleanup from the first | The stale release is rejected and cannot clear or return the second attack's fresh reservation |
| Token cleanup | Complete and cancel every move; also Sever an active Horn Charge and stream/remove the Hound controller | Newly claimed tokens are returned exactly once on every path; no stale target-ASC entry remains |
| Reserved target crossing | Reserve player A, then place hostile player B across the Bite/charge/pounce sweep | Only player A can take that activation's damage; player B cannot borrow A's attacker slot |
| Wrong controller | Possess the Hound with a controller not derived from `ANarrativeNPCController` | All three moves reject before movement, damage, token, or cooldown commitment |
...
| Synchronous callback re-entry | From a server-side Started/Impact or launch callback, cancel and immediately request the move again | Old callback continuations and epoch-bound timers cannot mutate, finish, or release the new activation |
