...
namespace
{
    ...
    public:
        ...
        bool bFailHint = false;
        TFunction<void(const FString&)> BeforeRead;
        TFunction<void(const FString&)> BeforeWrite;
        ...
        { if (BeforeRead) { BeforeRead(Slot); } if (const auto* Data = Files.Find(FString::FromInt(User) + Slot)) { Bytes = *Data; return true; } return false; }
        bool Exists(const FString& Slot, int32 User) override { return Files.Contains(FString::FromInt(User) + Slot); }
        bool Remove(const FString& Slot, int32 User) override { return Files.Remove(FString::FromInt(User) + Slot) > 0; }
        bool Write(const FString& Slot, int32 User, const TArray<uint8>& Bytes) override
        {
            if (BeforeWrite) { BeforeWrite(Slot); }
            if (bFailHint && Slot.Contains(TEXT("SovAccount_"))) { return false; }
    ...
    public:
        ...
        bool bHistory = false;
        TArray<FListComplete> Lists;
        TArray<FString> ReadRevisionIds;
        TArray<FString> DeletedRevisions;
        ...
        bool SupportsRevisionHistory() const override { return bHistory; }
        bool ListRevisions(FGuid, const FString&, FListComplete Callback) override { Lists.Add(MoveTemp(Callback)); return true; }
        bool ReadRevision(FGuid, const FString&, const FString& Revision, FReadComplete Callback) override
        { ReadRevisionIds.Add(Revision); Reads.Add(MoveTemp(Callback)); return true; }
        bool DeleteRevision(FGuid, const FString&, const FString& Revision, FWriteComplete Callback) override
        { DeletedRevisions.Add(Revision); Writes.Add(MoveTemp(Callback)); return true; }
...
struct FSovPlatformServicesTestAccess
{
    static FSovCloudTestStorage* Initialize(USovPlatformServicesSubsystem& Service, USovSaveSubsystem& Save,
        const TSharedPtr<FSovCloudTestAdapter, ESPMode::ThreadSafe>& Adapter)
    {
        ...
        Adapter->Start([&Service]() { Service.ObserveAccount(true); });
    ...
    { TStrongObjectPtr<USovCampaignSaveGame> Retained(Envelope); TGuardValue<bool> Mutation(Save.bBusy, true); FString Error; return Save.WriteEnvelope(Retained.Get(), Error) == ESovSaveResult::Success; }
    ...
    static void BindUnknown(USovPlatformServicesSubsystem& Service, USovSaveSubsystem& Save, const TSharedPtr<FSovCloudTestAdapter, ESPMode::ThreadSafe>& Adapter)
    ...
    static bool MissionReceiptCurrent(const USovSaveSubsystem& Save) { return Save.IsStorageOwnerCurrent(Save.MissionTravelOwner); }
    static void FenceAndRestoreSameOwner(USovSaveSubsystem& Save, const FSovObservedPlatformAccount& Account)
    { Save.FenceUnresolvedAccountChange(); Save.ObserveNativePlatformAccount(Account); }
    static void ServiceTick(USovPlatformServicesSubsystem& Service) { Service.Tick(0.1f); }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovSaveOwnerBoundaryReentryRuntime, "ProjectVelkorran.Campaign.PlatformServices.OwnerReceiptAcrossNativeIOAndSelection",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FSovSaveOwnerBoundaryReentryRuntime::RunTest(const FString&)
{
    ...
    auto Adapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>();
    Adapter->Account.bRequiresKnownStorageOwner = true; Adapter->Account.bStorageAccessAuthorized = true;
    auto* Storage = FSovPlatformServicesTestAccess::Initialize(*Service, *Save, Adapter);
    TestTrue(TEXT("Baseline native bank commits"), FSovPlatformServicesTestAccess::Write(*Save, FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("original"))));
    const auto Before = Storage->Files; bool Injected = false;
    Storage->BeforeRead = [&](const FString& Slot)
    {
        if (Injected || !Slot.Contains(TEXT("SovCampaign_"))) { return; }
        Injected = true;
        Adapter->Account.bStorageAccessAuthorized = false; Adapter->Changed();
        Adapter->Account.bStorageAccessAuthorized = true; Adapter->Changed();
    };
    TestFalse(TEXT("Native writer rejects revoke/re-authorize ABA during existing bank read"),
        FSovPlatformServicesTestAccess::Write(*Save, FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("must-not-write"))));
    TestTrue(TEXT("Original namespace is still available but the old operation receipt expired"), Injected && Save->IsPlatformStorageOwnerAvailable());
    TestTrue(TEXT("No later archive or bank write escaped the expired receipt"), Storage->Files.OrderIndependentCompareEqual(Before));
    Storage->BeforeRead = nullptr;

    FString Error;
    auto* Destination = NewObject<USovCampaignDefinition>(Instance.Get());
    TestTrue(TEXT("Persistent mission travel acquires the verified native owner"), Save->BeginMissionTravel(Destination, Error));
    const auto BeforeSuspend = Save->CaptureStorageOwner();
    Save->SetPlatformSuspended(true); Save->SetPlatformSuspended(false);
    TestTrue(TEXT("Persistent travel receipt survives only a clean suspend/resume"), FSovPlatformServicesTestAccess::MissionReceiptCurrent(*Save));
    TestFalse(TEXT("Ordinary I/O receipt never survives suspend/resume"), Save->IsStorageOwnerCurrent(BeforeSuspend));
    Save->SetPlatformSuspended(true);
    Adapter->Account.bStorageAccessAuthorized = false; Adapter->Changed();
    Adapter->Account.bStorageAccessAuthorized = true; Adapter->Changed();
    Save->SetPlatformSuspended(false);
    TestFalse(TEXT("Resume cannot repair a persistent receipt revoked and re-authorized during suspension"), FSovPlatformServicesTestAccess::MissionReceiptCurrent(*Save));
    Save->CancelMissionTravel();

    TestTrue(TEXT("Explicit cloud opt-in reacquires the available owner"), Service->SetCloudEnabled(true, Error));
    TestTrue(TEXT("Cloud compare captures the exact current owner generation"), Service->InspectCloudSlot(ESovSaveSlotKind::Manual, 0, Error));
    const auto CloudRequest = Service->GetCloudReview().RequestId;
    TArray<uint8> Remote;
    USovCampaignSaveGame::EncodeFramed(FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("late-old-owner")), Remote, Error);
    FSovPlatformServicesTestAccess::FenceAndRestoreSameOwner(*Save, Adapter->Account);
    Adapter->Reads.Last()(true, true, Remote, {});
    TestTrue(TEXT("Same-namespace stale cloud callback cannot publish a choice"), Service->GetCloudReview().RequestId == CloudRequest
        && Service->GetCloudReview().Phase == ESovCloudPhase::Reading);
    FSovPlatformServicesTestAccess::ServiceTick(*Service);
    TestEqual(TEXT("Owner generation expiry retires even a same-account cloud review"), Service->GetCloudReview().Phase, ESovCloudPhase::Cancelled);

    // A failed automatic profile-hint write leaves a legitimate direct public retry path.
    Storage->bFailHint = true; Adapter->ChangeTo(TEXT("TestProvider|AccountB"), true);
    Storage->bFailHint = false; bool SelectionRevoked = false;
    Storage->BeforeRead = [&](const FString& Slot)
    {
        if (SelectionRevoked || !Slot.Contains(TEXT("SovAccount_"))) { return; }
        SelectionRevoked = true; Adapter->Account.bStorageAccessAuthorized = false; Adapter->Changed();
    };
    TestFalse(TEXT("Direct profile retry cannot republish availability after revocation in hint I/O"),
        Save->SelectPlatformUser(TEXT("TestProvider|AccountB"), Adapter->Account.LocalUser, Error));
    TestTrue(TEXT("The original account remains fenced after rejected selection"), SelectionRevoked && !Save->IsPlatformStorageOwnerAvailable());
    Storage->BeforeRead = nullptr;
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovCloudRevisionHistoryRuntime, "ProjectVelkorran.Campaign.PlatformServices.ExplicitRevisionHistoryAndWorkerCompletion",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FSovCloudRevisionHistoryRuntime::RunTest(const FString&)
{
    TStrongObjectPtr<UGameInstance> Instance(NewObject<UGameInstance>());
    TStrongObjectPtr<USovSaveSubsystem> Save(NewObject<USovSaveSubsystem>(Instance.Get()));
    TStrongObjectPtr<USovPlatformServicesSubsystem> Service(NewObject<USovPlatformServicesSubsystem>(Instance.Get()));
    auto Adapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>(); Adapter->bHistory = true;
    FSovPlatformServicesTestAccess::Initialize(*Service, *Save, Adapter);
    FSovPlatformServicesTestAccess::Write(*Save, FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("local")));
    FString Error; Service->SetCloudEnabled(true, Error);
    TestTrue(TEXT("Cloud history inspection starts"), Service->InspectCloudSlot(ESovSaveSlotKind::Manual, 0, Error));
    const FString Ahead = TEXT("SovCloud1_0_0_0639343584000000000_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.sav");
    const FString Later = TEXT("SovCloud1_0_0_0639242496000000000_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.sav");
    TArray<uint8> A, B;
    USovCampaignSaveGame::EncodeFramed(FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("older-ahead-clock")), A, Error);
    USovCampaignSaveGame::EncodeFramed(FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("newer-corrected-clock")), B, Error);
    Adapter->Lists[0](true, {{ Ahead, A.Num() }, { Later, B.Num() }}, {});
    if (!TestEqual(TEXT("History reads the first candidate"), Adapter->Reads.Num(), 1)) { return false; }
    Adapter->Reads[0](true, true, A, {});
    FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread);
    if (!TestEqual(TEXT("History reads each candidate without retaining both payloads"), Adapter->Reads.Num(), 2)) { return false; }
    Adapter->Reads[1](true, true, B, {});
    FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread);
    const auto Review = Service->GetCloudReview();
    TestTrue(TEXT("Two verified candidates require explicit selection"), Review.Phase == ESovCloudPhase::AwaitingChoice
        && Review.bRevisionSelectionRequired && Review.SelectedRevisionId.IsEmpty() && Service->GetCloudRevisions().Num() == 2);
    TestFalse(TEXT("Use Cloud cannot silently choose the ahead-clock file"), Service->ResolveCloudReview(Review.RequestId, ESovCloudChoice::UseCloud, Error));
    TestTrue(TEXT("Corrected-clock later revision remains directly selectable"), Service->SelectCloudRevision(Review.RequestId, Later, Error));
    if (!TestEqual(TEXT("Selection requests the exact immutable filename"), Adapter->ReadRevisionIds.Last(), Later)) { return false; }
    auto Completion = MoveTemp(Adapter->Reads.Last());
    auto Worker = Async(EAsyncExecution::ThreadPool, [Completion = MoveTemp(Completion), B]() mutable { Completion(true, true, B, {}); });
    Worker.Wait(); FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread);
    TestTrue(TEXT("Worker completion is consumed on game thread with the exact selection"), Service->GetCloudReview().SelectedRevisionId == Later
        && !Service->GetCloudReview().bRevisionSelectionRequired && Service->GetCloudReview().Cloud.BoundaryId == FName(TEXT("newer-corrected-clock")));
    TestTrue(TEXT("Verified local bank permits explicitly confirmed retention cleanup"), Service->DeleteCloudRevision(Review.RequestId, Ahead, Error));
    TestEqual(TEXT("Cleanup targets only the reviewed immutable revision"), Adapter->DeletedRevisions.Last(), Ahead);
    Adapter->Writes.Last()(true, {});
    TestEqual(TEXT("Cleanup retires review for a fresh enumeration"), Service->GetCloudReview().Phase, ESovCloudPhase::Completed);
    return true;
}
...
bool FSovPlatformRevocationReentryRuntime::RunTest(const FString&)
{
    ...
    auto Adapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>(); auto* Storage = FSovPlatformServicesTestAccess::Initialize(*Service, *Save, Adapter);
...
bool FSovPlatformAccountRuntime::RunTest(const FString&)
{
    ...
    TStrongObjectPtr<USovSaveSubsystem> Save(NewObject<USovSaveSubsystem>(Instance.Get()));
    TStrongObjectPtr<USovPlatformServicesSubsystem> Service(NewObject<USovPlatformServicesSubsystem>(Instance.Get()));
    auto Adapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>(); FSovPlatformServicesTestAccess::Initialize(*Service, *Save, Adapter);
...
bool FSovOfflineProfileRestartRuntime::RunTest(const FString&)
{
    ...
    auto FirstAdapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>(); auto* FirstStorage = FSovPlatformServicesTestAccess::Initialize(*FirstService, *FirstSave, FirstAdapter);
    ...
    auto UnknownAdapter = MakeShared<FSovCloudTestAdapter, ESPMode::ThreadSafe>();
...
bool FSovCloudSaveAuthorityRuntime::RunTest(const FString&)
{
    ...
    Storage->bTornBank = false;
    // The torn inactive bank is archived into a bounded, explicitly manageable support slot.
    TestTrue(TEXT("A later verified native save repairs the torn bank"), FSovPlatformServicesTestAccess::Write(*Save,
        FSovPlatformServicesTestAccess::Envelope(*Save, TEXT("after-torn"))));
    const auto Archives = Save->ListPreservedSaveArchives(ESovSaveSlotKind::Manual, 0);
    TestEqual(TEXT("The preserved corrupt bank is discoverable without enumerating unrelated user files"), Archives.Num(), 1);
    if (Archives.Num() == 1)
    {
        TestTrue(TEXT("Explicit archive cleanup retains a separate verified campaign bank"), Save->DeletePreservedSaveArchive(ESovSaveSlotKind::Manual, 0, Archives[0], Error));
        TestTrue(TEXT("Deleted archive is no longer offered"), Save->ListPreservedSaveArchives(ESovSaveSlotKind::Manual, 0).IsEmpty());
    }
    return true;
}
