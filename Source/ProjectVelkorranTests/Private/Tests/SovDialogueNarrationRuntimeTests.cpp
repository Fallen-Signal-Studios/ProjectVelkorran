...
struct FSovDialogueTestAccess
{
	...
	static void StageRemoval(USovDialoguePresentationComponent* C, UTalesComponent* Tales,UDialogue* Dialogue,USovDialogueChoiceWidget* Widget)
    {
        C->Tales=Tales; C->SeenDialogue=Dialogue; C->SeenRevision=Dialogue->GetReplyPresentationRevision();
        C->PresentedDialogue=Dialogue; C->ChoiceWidget=Widget;
        Widget->OnPresentationRemoved.BindUObject(C,&USovDialoguePresentationComponent::OnWidgetRemoved);
    }
    static int64 Seen(USovDialoguePresentationComponent* C) { return C->SeenRevision; }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovDialogueRemovedViewRepair,"ProjectVelkorran.UI.Dialogue.RemovedViewDoesNotConsumeReplyRevision",EAutomationTestFlags::EditorContext|EAutomationTestFlags::ProductFilter)
bool FSovDialogueRemovedViewRepair::RunTest(const FString&)
{
    FDialogueWorld W; auto* Dialogue=NewObject<USovDialogueRuntimeFixture>(W.PC); Dialogue->Stage(W.Tales); Dialogue->NPCFinishedTalking();
    const int64 Revision=Dialogue->GetReplyPresentationRevision();
    auto* Presenter=NewObject<USovDialoguePresentationComponent>(W.PC);
    auto* Widget=NewObject<USovDialogueChoiceWidget>(); Widget->Initialize(); Widget->TakeWidget();
    FSovDialogueTestAccess::StageRemoval(Presenter,W.Tales,Dialogue,Widget);
    Widget->Retire();
    TestEqual(TEXT("Real widget removal invalidates the presenter's seen revision for retry"),FSovDialogueTestAccess::Seen(Presenter),int64(INDEX_NONE));
    TestTrue(TEXT("View destruction does not consume legal Narrative responses"),Dialogue->IsCurrentReplyPresentation(Revision));
    TestFalse(TEXT("Removed text cannot keep pressure running"),FSovDialogueTestAccess::HasLivePressure(Presenter));
    return true;
}
