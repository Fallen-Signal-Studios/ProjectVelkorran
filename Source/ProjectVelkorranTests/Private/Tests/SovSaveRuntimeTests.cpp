...
struct FSovSaveTestAccess
{
    ...
    static void FailedBoundary(USovSaveSubsystem& S, USovCampaignDefinition* Mission, FName Boundary)
    {
        S.FailedWrite = NewObject<USovCampaignSaveGame>(&S); S.bAwaitingFailureDecision = true;
        auto& H = S.FailedWrite->Header; H.Kind = ESovSaveSlotKind::Checkpoint;
        H.MissionId = Mission->MissionId; H.ActiveProtagonist = Mission->Protagonist;
        H.BoundaryKind = ESovSaveBoundary::CanonGate; H.BoundaryId = Boundary;
    }
    static void ExpireBoundary(USovSaveSubsystem& S) { S.AcknowledgmentExpiresAt = FPlatformTime::Seconds() - 1.; }
    static void ExpireMissionTravel(USovSaveSubsystem& S) { S.MissionTravelDeadline = FPlatformTime::Seconds() - 1.; }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovSaveBoundaryAcknowledgementRuntime, "ProjectVelkorran.Campaign.Save.ExactAcknowledgedBoundaryAndAbandonment",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FSovSaveBoundaryAcknowledgementRuntime::RunTest(const FString&)
{
    ...
    auto* PC = World->SpawnActor<ASovSaveReceiptTestController>();
    auto* Pawn = World->SpawnActor<ASovCampaignRuntimeTestPawn>();
    TStrongObjectPtr<USovSaveReceiptTestSubsystem> Save(NewObject<USovSaveReceiptTestSubsystem>());
    if (PC && Pawn)
    {
        PC->Possess(Pawn); Save->TestController = PC; FSovSaveTestAccess::Initialize(*Save);
        auto* Mission = NewObject<USovCampaignDefinition>(PC);
        Mission->MissionId = TEXT("ReceiptMission"); Mission->Protagonist = Pawn->TestHero;
        Mission->PawnClass = ASovCampaignRuntimeTestPawn::StaticClass(); Mission->PlayerDefinition = NewObject<UPlayerDefinition>(PC);
        FSovCampaignBeatDefinition Beat; Beat.BeatId = TEXT("ReceiptBeat"); Mission->Beats.Add(Beat);
        TestEqual(TEXT("Actual possessed campaign state starts"), PC->GetCampaignState()->BeginMission(Mission), ESovCampaignResult::Applied);
        FSovSaveTestAccess::FailedBoundary(*Save, Mission, Beat.BeatId); Save->AcknowledgeSaveFailure();
        TestFalse(TEXT("Other boundary ID cannot consume the acknowledgement"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::CanonGate, TEXT("Other")));
        TestFalse(TEXT("Other boundary kind cannot consume it"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::ArenaEntry, Beat.BeatId));
        TestTrue(TEXT("Exact mission/hero/world/boundary may continue once"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::CanonGate, Beat.BeatId));
        TestFalse(TEXT("Acknowledgement cannot be replayed"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::CanonGate, Beat.BeatId));
        FSovSaveTestAccess::FailedBoundary(*Save, Mission, Beat.BeatId); Save->AcknowledgeSaveFailure(); FSovSaveTestAccess::ExpireBoundary(*Save);
        TestFalse(TEXT("Expired acknowledgement cannot skip a later save"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::CanonGate, Beat.BeatId));
        TStrongObjectPtr<USovSaveLoadCompletionProbe> Probe(NewObject<USovSaveLoadCompletionProbe>());
        Probe->Subsystem = Save.Get(); Save->OnLoadCompleted.AddDynamic(Probe.Get(), &USovSaveLoadCompletionProbe::OnCompleted);
        FString TravelError;
        TestTrue(TEXT("Existing GI save owner accepts the mission-travel transaction"), Save->BeginMissionTravel(Mission, TravelError));
        FSovSaveTestAccess::ExpireMissionTravel(*Save); FSovSaveTestAccess::Tick(*Save);
        TestEqual(TEXT("Expired mission travel publishes one actionable recovery result"), Probe->Notifications, 1);
        TestEqual(TEXT("No checkpoint is never reported as successful recovery"), Probe->LastResult, ESovSaveResult::RecoveryAvailable);
        TestEqual(TEXT("Native controller leaves the wedged travelling state"), PC->GetCampaignTransitionState(), ESovCampaignTransitionState::Failed);
        FSovSaveTestAccess::Tick(*Save);
        TestEqual(TEXT("Failed recovery cannot recursively retry every frame"), Probe->Notifications, 1);
        FSovSaveTestAccess::FailedBoundary(*Save, Mission, Beat.BeatId); Save->AcknowledgeSaveFailure();
        FString Error; TestTrue(TEXT("Title abandonment retires transient ownership without I/O"), Save->AbandonSessionForTitle(Error));
        TestFalse(TEXT("Abandonment clears acknowledged continuation"), Save->ConsumeAcknowledgedBoundary(ESovSaveBoundary::CanonGate, Beat.BeatId));
        TestFalse(TEXT("Abandoned campaign storage cannot reopen from an old callback"), Save->IsPlatformStorageOwnerAvailable());
    }
    else { AddError(TEXT("Acknowledgement world failed to spawn its native owner")); }
    Save->TestController = nullptr;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovSaveFrameMigrationTest, "ProjectVelkorran.Campaign.Save.RawFrameAndCurrentUnframedMigration",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FSovSaveFrameMigrationTest::RunTest(const FString&)
{
    TStrongObjectPtr<USovSaveSubsystem> Save(NewObject<USovSaveSubsystem>());
    auto* Storage = FSovSaveTestAccess::Initialize(*Save); FString Error;
    TStrongObjectPtr<USovCampaignSaveGame> Legacy(FSovSaveTestAccess::Envelope(*Save));
    Legacy->Header.Generation = 7; Legacy->IntegrityChecksum = Legacy->CalculateChecksum();
    TArray<uint8> GenuineLegacy;
    TestTrue(TEXT("Current engine generates the actual pre-frame bank format"), UGameplayStatics::SaveGameToMemory(Legacy.Get(), GenuineLegacy));
    const FString OriginalSlot = FString::FromInt(Save->GetLocalSaveUserIndex()) + FSovSaveTestAccess::Name(*Save, ESovSaveSlotKind::Manual, 0, 0);
    Storage->Slots.Add(OriginalSlot, GenuineLegacy);
    bool Damaged = false;
    TStrongObjectPtr<USovCampaignSaveGame> Decoded(FSovSaveTestAccess::Read(*Save, ESovSaveSlotKind::Manual, 0, Damaged));
    if (!TestTrue(TEXT("Genuine current unframed bank remains compatible"), Decoded.IsValid() && Decoded->bReadLegacyUnframed && !Damaged)) { return false; }
    TStrongObjectPtr<USovCampaignSaveGame> Next(FSovSaveTestAccess::Envelope(*Save));
    TestEqual(TEXT("Next verified write migrates into alternate framed bank"), FSovSaveTestAccess::Write(*Save, Next.Get(), Error), ESovSaveResult::Success);
    TestTrue(TEXT("The original migration source is preserved byte for byte"), Storage->Slots.FindChecked(OriginalSlot) == GenuineLegacy);
    Decoded.Reset(FSovSaveTestAccess::Read(*Save, ESovSaveSlotKind::Manual, 0, Damaged));
    TestTrue(TEXT("Framed successor records the real migration"), Decoded.IsValid() && !Decoded->bReadLegacyUnframed
        && Decoded->Header.Generation == 8 && Decoded->Header.MigrationHistory.Contains(TEXT("Envelope.RawUnframedToSVF2.v1")));
    TArray<uint8> Framed;
    TestTrue(TEXT("Framed encoder accepts exact envelope class"), USovCampaignSaveGame::EncodeFramed(Next.Get(), Framed, Error));
    if (!Framed.IsEmpty()) { Framed.Last() ^= 0x40; }
    TestNull(TEXT("Raw corruption is rejected before UObject deserialization"), USovCampaignSaveGame::DecodeFramed(Framed, Error));
    TestNull(TEXT("Nested Narrative save cannot select itself as envelope class"), USovCampaignSaveGame::DecodeFramed(Legacy->NarrativePayload, Error));
    GenuineLegacy[4] = 99;
    TestNull(TEXT("Unknown engine header format is retained and rejected"), USovCampaignSaveGame::DecodeFramed(GenuineLegacy, Error));
    return true;
}
