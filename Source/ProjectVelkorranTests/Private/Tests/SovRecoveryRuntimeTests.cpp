...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovRecoveryRequiredParticipantFailureTest,
	"ProjectVelkorran.Campaign.Recovery.RequiredActorLossUsesOwnedLivingRetry",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
bool FSovRecoveryRequiredParticipantFailureTest::RunTest(const FString& Parameters)
{
	FRecoveryWorld F; if (!TestNotNull(TEXT("Ready persistent ASC"), F.ASC)) { return false; }
	auto* Recovery = F.Pawn->GetRecoveryComponent();
	auto* Encounter = F.World->SpawnActor<ASovEncounterRuntimeTestDirector>();
	if (!TestNotNull(TEXT("Owning encounter"), Encounter)) { return false; }
	Encounter->SeedState(ESovEncounterState::Failed);
	TestFalse(TEXT("A director without an entry checkpoint cannot acquire recovery"), Recovery->RequestEncounterFailure(Encounter, TEXT("Missing participant")));
	Encounter->SeedRecoveryCheckpoint();
	const auto Busy = FNarrativeGameplayTags::Get().State_Busy; F.ASC->AddLooseGameplayTag(Busy);
	TestTrue(TEXT("Failed checkpoint owner acquires the existing retry component"), Recovery->RequestEncounterFailure(Encounter, TEXT("Required participant was lost")));
	TestEqual(TEXT("Living failure enters retry without death/rescue fabrication"), Recovery->GetRecoveryState(), ESovRecoveryState::Retrying);
	TestTrue(TEXT("Player remains alive during encounter failure"), F.Pawn->IsAlive());
	TestEqual(TEXT("Recovery owns exactly one additional Busy lease"), F.ASC->GetTagCount(Busy), 2);
	TestTrue(TEXT("Recovery owns its movement lock"), F.PC->IsMoveInputIgnored());
	TestFalse(TEXT("Duplicate actor-loss notification cannot enqueue another retry"), Recovery->RequestEncounterFailure(Encounter, TEXT("Duplicate")));
	FSovRecoveryTestAccess::Detach(Recovery);
	TestEqual(TEXT("Detaching recovery preserves unrelated Busy ownership"), F.ASC->GetTagCount(Busy), 1);
	TestFalse(TEXT("Detaching releases its movement lock"), F.PC->IsMoveInputIgnored());
	return true;
}
#endif
