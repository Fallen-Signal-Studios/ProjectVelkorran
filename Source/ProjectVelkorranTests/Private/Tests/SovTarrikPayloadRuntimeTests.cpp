...
template<class T> T* TarrikPayloadActivate(FAutomationTestBase& Test, ASovAxiomRuntimeTestCharacter* Source)
{
...
float TarrikPayloadShield(ASovAxiomRuntimeTestCharacter* Actor)
{
...
bool FSovTarrikSlamRuntimeTest::RunTest(const FString& Parameters)
{
	...
	auto* Ability = TarrikPayloadActivate<USovTarrikSlamTestAbility>(*this, Source);
	...
	TestTrue(TEXT("Hostile Shield reduced"), TarrikPayloadShield(Target) < 100.f);
	TestEqual(TEXT("Friendly unchanged"), TarrikPayloadShield(Friendly), 100.f);
	TestEqual(TEXT("Occluded target unchanged"), TarrikPayloadShield(Hidden), 100.f);
	TestEqual(TEXT("Immune target unchanged"), TarrikPayloadShield(Immune), 100.f);
	...
	TestEqual(TEXT("Ward does not grant Shield"), TarrikPayloadShield(Source), 100.f);
...
bool FSovTarrikRequiemRuntimeTest::RunTest(const FString& Parameters)
{
	...
	auto* Ability = TarrikPayloadActivate<USovTarrikRequiemTestAbility>(*this, Source);
	...
	TestEqual(TEXT("Friendly does not absorb or receive penetration"), TarrikPayloadShield(Friendly), 100.f);
	TestEqual(TEXT("World cover stops penetration"), TarrikPayloadShield(Hidden), 100.f);
	...
	TestEqual(TEXT("World cover also stops chained blasts"), TarrikPayloadShield(Hidden), 100.f);
...
{
	{
		FTarrikTestWorld Fixture;
		auto* Source = Fixture.Character(FVector::ZeroVector, 0);
		auto* Hunger = TarrikPayloadActivate<USovTarrikHungerTestAbility>(*this, Source);
	...
	}
	{
		FTarrikTestWorld Fixture;
		auto* Source = Fixture.Character(FVector::ZeroVector, 0);
		auto* Grenade = TarrikPayloadActivate<USovTarrikGrenadeTestAbility>(*this, Source);
