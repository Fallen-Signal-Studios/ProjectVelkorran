...
struct FSovNarrativeCueTestAccess
{
	...
	static void Expire(USovNarrativeCueComponent* C) { C->BarkEndsAt=-1.; }
	static void ClearRetry(USovNarrativeCueComponent* C) { C->Pending.Reset(); C->NextAllowed.Reset(); }
	static void RetireWitness(USovNarrativeCueComponent* C) { C->CurrentRequest.Witness=FSovGameplayTags::Get().Character_Player_Selene; }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovDialoguePausedCompletionRepair,"ProjectVelkorran.Campaign.Narrative.PausedTerminalCallbackDeferredOnce",EAutomationTestFlags::EditorContext|EAutomationTestFlags::ProductFilter)
bool FSovDialoguePausedCompletionRepair::RunTest(const FString&)
{
    FCueWorld F; auto* Tales=F.PC->FindComponentByClass<UTalesComponent>();
    auto* Dialogue=NewObject<USovNarrativeCueRuntimeDialogue>(F.PC); F.PC->KeepAlive.Add(Dialogue); Dialogue->Stage(Tales);
    F.World->GetWorldSettings()->Pauser=F.PC->GetPlayerState<APlayerState>();
    TestTrue(TEXT("Fixture uses actual world pause"),F.World->IsPaused());
    Dialogue->FinishNPCDialogue(); Dialogue->EndCurrentLine(); Dialogue->PumpDeferredLineCompletion();
    TestEqual(TEXT("Paused audio/sequence completion cannot advance Narrative"),Dialogue->FinishNotifications,0);
    F.World->GetWorldSettings()->Pauser=nullptr;
    Dialogue->SetPlaybackSuspended(true); Dialogue->PumpDeferredLineCompletion();
    TestEqual(TEXT("A second explicit suspension retains the deferred terminal event"),Dialogue->FinishNotifications,0);
    Dialogue->SetPlaybackSuspended(false); Dialogue->PumpDeferredLineCompletion(); Dialogue->PumpDeferredLineCompletion();
    TestEqual(TEXT("Terminal completion resumes exactly once instead of being dropped"),Dialogue->FinishNotifications,1);
    Dialogue->Deinitialize();
    return true;
}
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovCueReadHoldAndArchiveRepair,"ProjectVelkorran.Campaign.Narrative.SubtitleHoldAndWitnessedArchive",EAutomationTestFlags::EditorContext|EAutomationTestFlags::ProductFilter)
bool FSovCueReadHoldAndArchiveRepair::RunTest(const FString&)
{
    FCueWorld F; auto* Cues=F.PC->GetNarrativeCues(); FString Error;
    auto* Critical=F.Cue(TEXT("RecordedDirection"),ESovNarrativeCuePriority::ObjectiveCritical);
    Critical->bCritical=true; Critical->bRecordUnheardSummary=true; Critical->RecordSummary=FText::FromString(TEXT("An acquired direction."));
    TestTrue(TEXT("Critical cue queues"),Cues->RequestCue(Critical,F.Pawn,Error)); FSovNarrativeCueTestAccess::Tick(Cues);
    UObject* Reader=NewObject<UObject>(F.PC); F.PC->KeepAlive.Add(Reader);
    Cues->SetBarkSubtitleHold(Critical,Reader,true); FSovNarrativeCueTestAccess::Expire(Cues); FSovNarrativeCueTestAccess::Tick(Cues);
    TestTrue(TEXT("Unread caption ownership holds the real cue arbiter"),FSovNarrativeCueTestAccess::Playing(Cues)==Critical);
    Cues->SetBarkSubtitleHold(Critical,Reader,false); FSovNarrativeCueTestAccess::Tick(Cues);
    TestNull(TEXT("Released reader permits cue completion"),FSovNarrativeCueTestAccess::Playing(Cues));
    FSovNarrativeCueTestAccess::ClearRetry(Cues); Cues->RequestCue(Critical,F.Pawn,Error); FSovNarrativeCueTestAccess::Tick(Cues); FSovNarrativeCueTestAccess::Interrupt(Cues);
    TestTrue(TEXT("Interrupted critical summary belongs to its witnessed protagonist"),Cues->GetUnheardRecords().Contains(Critical));
    Critical->RequiredKnowledge.AddTag(FSovGameplayTags::Get().Character_Player_Selene);
    TestFalse(TEXT("Record producer enforces knowledge before summary disclosure"),Cues->GetUnheardRecords().Contains(Critical));
    TestFalse(TEXT("Replay cannot bypass record knowledge"),Cues->ReplayUnheardRecord(Critical,Error));
    Critical->RequiredKnowledge.Reset(); FSovNarrativeCueTestAccess::ClearRetry(Cues);
    TestTrue(TEXT("Eligible native record action requeues through the existing cue system"),Cues->ReplayUnheardRecord(Critical,Error));
    FSovNarrativeCueTestAccess::Tick(Cues); FSovNarrativeCueTestAccess::RetireWitness(Cues); FSovNarrativeCueTestAccess::Tick(Cues);
    TestNull(TEXT("A live generic bark cannot continue for another protagonist"),FSovNarrativeCueTestAccess::Playing(Cues));
    return true;
}
