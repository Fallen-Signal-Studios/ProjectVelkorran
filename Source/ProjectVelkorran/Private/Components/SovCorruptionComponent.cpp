...
bool USovCorruptionComponent::ValidOwner() const
{
	...
	return !bEnding && IsValid(Owner) && Owner->HasAuthority() && !Owner->IsActorBeingDestroyed()
		&& !Owner->FindComponentByClass<USovLegacyCorruptionComponent>()
...
{
	if (GetOwner() && GetOwner()->FindComponentByClass<USovLegacyCorruptionComponent>())
	{
		OutError = TEXT("Remove the opt-in legacy corruption component before entering a managed campaign; migrate its fields to mission-permitted profiles.");
		return false;
	}
	const auto* StateComponent = Campaign();
