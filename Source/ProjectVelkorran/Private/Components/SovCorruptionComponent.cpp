...
bool FSovCorruptionSourceSpec::HasValidNumbers() const
{
	...
	if ((FalloffPolicy != ESovCorruptionFalloffPolicy::None
		...
		|| static_cast<uint8>(BandCap)
			> static_cast<uint8>(ESovCorruptionBand::OverwriteRisk)
		|| (bEnforceBandCap && BandCap == ESovCorruptionBand::None))
	{
		return false;
	}
...
void USovCorruptionComponent::BeginPlay()
{
	Super::BeginPlay();
...
void USovCorruptionComponent::EndPlay(
...
void USovCorruptionComponent::GetLifetimeReplicatedProps(
	TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(USovCorruptionComponent, CurrentBand);
	DOREPLIFETIME(USovCorruptionComponent, PresentationState);
	DOREPLIFETIME(USovCorruptionComponent, bMissionAllowsOverwriteRisk);
	DOREPLIFETIME(
		USovCorruptionComponent,
		bOverwriteRiskAuthorizedForCurrentExposure);
}

bool USovCorruptionComponent::InitializeWithAbilitySystem(
	UAbilitySystemComponent* InAbilitySystemComponent)
{
	...
	if (bReplacingAbilitySystem)
	{
		...
		CurrentBand = ESovCorruptionBand::None;
...
bool USovCorruptionComponent::IsInitialized() const
{
...
float USovCorruptionComponent::GetCorruption() const
{
...
float USovCorruptionComponent::GetMaxCorruption() const
{
	return IsInitialized()
		? FMath::Max(
			AbilitySystemComponent->GetNumericAttribute(
...
float USovCorruptionComponent::GetNormalizedCorruption() const
{
	return FMath::Clamp(GetCorruption() / GetMaxCorruption(), 0.0f, 1.0f);
}

ESovCorruptionBand USovCorruptionComponent::DetermineBandForExposure(
	const float Exposure) const
{
	...
	{
		return ESovCorruptionBand::None;
	}

	if (ClampedExposure
		>= GetThresholdForBand(ESovCorruptionBand::OverwriteRisk)
		&& bMissionAllowsOverwriteRisk
		&& bOverwriteRiskAuthorizedForCurrentExposure)
	{
		return ESovCorruptionBand::OverwriteRisk;
	}
	if (ClampedExposure
		>= GetThresholdForBand(ESovCorruptionBand::Contest))
	{
		return ESovCorruptionBand::Contest;
	}
	if (ClampedExposure
		>= GetThresholdForBand(ESovCorruptionBand::Intrusion))
	{
		return ESovCorruptionBand::Intrusion;
	}
	return ESovCorruptionBand::Trace;
}

float USovCorruptionComponent::GetExposureCeilingForBand(
	const ESovCorruptionBand Band) const
{
	...
	switch (Band)
	{
	case ESovCorruptionBand::None:
		return 0.0f;
	case ESovCorruptionBand::Trace:
		return BeforeNextBand(
			GetThresholdForBand(ESovCorruptionBand::Intrusion));
	case ESovCorruptionBand::Intrusion:
		return BeforeNextBand(
			GetThresholdForBand(ESovCorruptionBand::Contest));
	case ESovCorruptionBand::Contest:
		return BeforeNextBand(
			GetThresholdForBand(ESovCorruptionBand::OverwriteRisk));
	case ESovCorruptionBand::OverwriteRisk:
	default:
		return MaxValue;
	}
}
...
USovCorruptionComponent::GetPresentationSnapshot() const
{
...
FSovCorruptionSourceHandle
USovCorruptionComponent::RegisterCorruptionSource(
	AActor* SourceActor,
	const FSovCorruptionSourceSpec& SourceSpec)
{
	FSovCorruptionSourceHandle Result;
...
bool USovCorruptionComponent::UpdateCorruptionSource(
...
bool USovCorruptionComponent::UnregisterCorruptionSource(
	const FSovCorruptionSourceHandle SourceHandle)
{
...
bool USovCorruptionComponent::HasRegisteredCorruptionSource(
	const FSovCorruptionSourceHandle SourceHandle) const
{
	return SourceHandle.IsValid()
		&& ActiveSources.Contains(SourceHandle.Id);
}

void USovCorruptionComponent::ClearCorruptionSources()
{
...
float USovCorruptionComponent::ApplyInstantExposure(
	const float ExposureAmount,
	AActor* SourceActor,
	const ESovCorruptionBand BandCap)
{
	if (!CanWriteCorruption()
	...
	const ESovCorruptionBand SafeBandCap =
		static_cast<uint8>(BandCap)
			< static_cast<uint8>(ESovCorruptionBand::Intrusion)
			? BandCap
...
float USovCorruptionComponent::ReduceCorruption(
...
bool USovCorruptionComponent::ClearCorruption(AActor* RemedyInstigator)
{
	if (!CanWriteCorruption())
	{
		return false;
...
bool USovCorruptionComponent::ApplyRemedy(
...
void USovCorruptionComponent::SetMissionAllowsOverwriteRisk(
	const bool bAllowed)
{
	if (!CanWriteCorruption(true)
...
void USovCorruptionComponent::SetReducedEffectsPresentationEnabled(
	const bool bEnabled)
{
...
USovCorruptionComponent::CaptureCorruptionCheckpoint(
...
void USovCorruptionComponent::FinalizeCorruptionSourceRestore()
{
	if (GetOwner() && GetOwner()->HasAuthority())
	{
		ClearInstantReplayGuards();
...
bool USovCorruptionComponent::RestoreCorruptionCheckpoint(
...
void USovCorruptionComponent::TryInitializeFromOwner()
{
	if (!IsValid(GetOwner()))
	{
		return;
...
void USovCorruptionComponent::HandleOwnerASCInitialized()
{
	TryInitializeFromOwner();
}

void USovCorruptionComponent::HandleDeathStateChanged(
	AActor* ChangedActor,
...
void USovCorruptionComponent::UninitializeFromAbilitySystem()
{
	StopSourceTimer();
	ClearPendingInstantExposures();
	ActiveSources.Reset();
...
bool USovCorruptionComponent::HasCurrentAvatarBinding() const
{
	return IsValid(AbilitySystemComponent)
		&& IsValid(GetOwner())
...
bool USovCorruptionComponent::CanWriteCorruption(const bool bAllowDead) const
{
...
void USovCorruptionComponent::HandleCorruptionAttributeChanged(
...
void USovCorruptionComponent::HandleMaxCorruptionAttributeChanged(
	const FOnAttributeChangeData& ChangeData)
{
	if (!HasCurrentAvatarBinding())
	{
...
void USovCorruptionComponent::SetCorruptionInternal(
...
void USovCorruptionComponent::SetMaxCorruptionInternal(
...
void USovCorruptionComponent::RefreshSourceTimer()
{
	if (!CanWriteCorruption())
	{
		StopSourceTimer();
...
void USovCorruptionComponent::StopSourceTimer()
{
	if (UWorld* World = GetWorld())
	{
...
void USovCorruptionComponent::EvaluateSourceTimer()
{
...
void USovCorruptionComponent::EvaluateAccumulatedSources(const bool bAllowDead)
{
	if (!CanWriteCorruption(bAllowDead))
	{
		StopSourceTimer();
	...
	for (const TPair<FGuid, FSovActiveSource>& Pair : ActiveSources)
	{
		...
		if (!bMayEnterOverwrite)
		{
			SourceCeiling = FMath::Min(
				SourceCeiling,
				GetExposureCeilingForBand(ESovCorruptionBand::Contest));
		}
...
void USovCorruptionComponent::SchedulePendingInstantExposureEvaluation()
{
	UWorld* World = GetWorld();
...
void USovCorruptionComponent::EvaluatePendingInstantExposures()
{
	if (UWorld* World = GetWorld())
	{
...
void USovCorruptionComponent::ClearPendingInstantExposures()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().ClearTimer(PendingInstantExposureTimerHandle);
...
float USovCorruptionComponent::CalculateSourceStrength(
...
bool USovCorruptionComponent::SourceAllowsTarget(
...
bool USovCorruptionComponent::SourceHasLineOfSight(
	const FSovActiveSource& ActiveSource) const
{
...
float USovCorruptionComponent::ApplyExposureInternal(
	const float ExposureAmount,
	const ESovCorruptionBand BandCap,
...
void USovCorruptionComponent::RefreshBand(const bool bBroadcastChanges)
{
	if (!IsInitialized())
	{
		return;
...
void USovCorruptionComponent::SetBand(
	const ESovCorruptionBand NewBand,
	const bool bBroadcastChanges)
{
	...
	const ESovCorruptionBand OldBand = CurrentBand;
	RemoveBandEffect();
	CurrentBand = NewBand;
	ApplyBandEffect();
	WakeForReplication();
	...
	if (bBroadcastChanges)
	{
		...
		SendCorruptionEvent(
			...
			GetOwner());
		if (NewBand == ESovCorruptionBand::OverwriteRisk)
		{
...
void USovCorruptionComponent::RemoveBandEffect()
{
...
void USovCorruptionComponent::ApplyBandEffect()
{
	if (!CanWriteCorruption()
		|| CurrentBand == ESovCorruptionBand::None
...
FGameplayTag USovCorruptionComponent::GetTagForBand(
	const ESovCorruptionBand Band) const
{
...
float USovCorruptionComponent::GetThresholdForBand(
	const ESovCorruptionBand Band) const
{
...
void USovCorruptionComponent::RefreshPresentationState()
{
	if (!GetOwner() || !GetOwner()->HasAuthority())
	{
		return;
...
void USovCorruptionComponent::RememberPresentationContract(
	const FSovCorruptionSourceSpec& SourceSpec)
{
...
void USovCorruptionComponent::ClearRememberedPresentationContract()
{
...
void USovCorruptionComponent::BroadcastPresentationSnapshot()
{
	OnCorruptionPresentationChanged.Broadcast(GetPresentationSnapshot());
}

void USovCorruptionComponent::WakeForReplication() const
{
...
void USovCorruptionComponent::SendCorruptionEvent(
...
void USovCorruptionComponent::ResetForDeath()
{
	if (!CanWriteCorruption(true))
	{
		return;
	...
	if (!bHasPresentationContract)
	{
		SafePostDeathCorruption = FMath::Min(
			SafePostDeathCorruption,
			GetExposureCeilingForBand(ESovCorruptionBand::Intrusion));
	}
...
bool USovCorruptionComponent::ApplyCheckpointNow(
	const FSovCorruptionCheckpointData& CheckpointData)
{
...
void USovCorruptionComponent::ScheduleInstantReplayGuardWarning()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().ClearTimer(InstantReplayGuardTimerHandle);
...
void USovCorruptionComponent::HandleInstantReplayGuardWarning()
{
	if (!InstantExposureReplayGuardSourceIds.IsEmpty())
	{
		UE_LOG(
...
void USovCorruptionComponent::ClearInstantReplayGuards()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().ClearTimer(InstantReplayGuardTimerHandle);
...
void USovCorruptionComponent::OnRep_CurrentBand(
	const ESovCorruptionBand OldBand)
{
	if (OldBand != CurrentBand)
	{
		OnCorruptionBandChanged.Broadcast(OldBand, CurrentBand);
...
void USovCorruptionComponent::OnRep_PresentationState(
