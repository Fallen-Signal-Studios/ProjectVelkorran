	...
	FGameplayTag AbilityTag, float Damage, float Poise, float Falloff = 1.f, bool* OutPoiseBroken = nullptr, bool bRequestBurn = false)
{
	...
	if (bRequestBurn)
	{
		Spec.Data->AddDynamicAssetTag(Tags.Status_Apply_Burn);
		Spec.Data->AddDynamicAssetTag(Tags.Status_Application_NativeOwned);
	}
	...
	FGameplayTag AbilityTag, float Damage, float Duration)
{
	...
	if (Owned.HasTagExact(Tags.Status_Immunity) || Owned.HasTag(Tags.Status_Immunity_All)) { return; }
