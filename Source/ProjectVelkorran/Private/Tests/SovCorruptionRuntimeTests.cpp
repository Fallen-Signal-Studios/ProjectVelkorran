...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSovCorruptionLegacyIsolationTest,
	"ProjectVelkorran.Campaign.Corruption.LegacyIsolation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
bool FSovCorruptionLegacyIsolationTest::RunTest(const FString& Parameters)
{
	FCorruptionWorld F;
	if (!F.Pawn || !F.PC) { AddError(TEXT("Fixture failed")); return false; }
	auto* ASC = F.Pawn->GetNarrativeAbilitySystemComponent();
	ASC->AddAttributeSetSubobject(NewObject<USovCorruptionAttributeSet>(F.Pawn));
	auto* Legacy = NewObject<USovLegacyCorruptionComponent>(F.Pawn);
	F.Pawn->AddInstanceComponent(Legacy); Legacy->RegisterComponent();
	TestTrue(TEXT("Explicit legacy component initializes outside a mission"), Legacy->InitializeWithAbilitySystem(ASC));
	auto* Mission = F.Mission(); F.PC->State->BeginMission(Mission);
	FString Error;
	TestFalse(TEXT("Managed restore refuses simultaneous corruption owners"), F.Pawn->GetCorruptionComponent()->FinishCampaignRestore(Mission, Error));
	TestFalse(TEXT("Legacy write cannot bypass active mission permission"), Legacy->ApplyInstantExposure(10.0f, F.Pawn, ESovLegacyCorruptionBand::Intrusion) > 0.0f);
	auto* Source = F.Source(F.Profile(TEXT("IsolatedField")));
	TestFalse(TEXT("Campaign source cannot accumulate alongside legacy ownership"), F.Pawn->GetCorruptionComponent()->HasExposureSource());
	Legacy->DestroyComponent();
	TestTrue(TEXT("Managed restore succeeds after legacy owner is removed"), F.Pawn->GetCorruptionComponent()->FinishCampaignRestore(Mission, Error));
	FSovCorruptionTestAccess::Refresh(*Source);
	TestTrue(TEXT("Mission-permitted source resumes with one owner"), F.Pawn->GetCorruptionComponent()->HasExposureSource());
	return true;
}
