...
bool FSovPassiveResourceReadyEpochTest::RunTest(const FString& Parameters)
{
	...
	auto* Shield = Attach<USovShieldComponent>(Owner, ASC);
	auto* Poise = Attach<USovPoiseComponent>(Owner, ASC);
	const uint64 OldShieldBinding = Shield->GetBindingGeneration();
	const uint64 OldPoiseBinding = Poise->GetBindingGeneration();
	Shield->SetCheckpointRestoreInProgress(true, OldShieldBinding);
	Poise->SetCheckpointRestoreInProgress(true, OldPoiseBinding);
	...
	TestTrue(TEXT("Shield binding generation advances"), Shield->GetBindingGeneration() != OldShieldBinding);
	TestTrue(TEXT("Poise binding generation advances"), Poise->GetBindingGeneration() != OldPoiseBinding);
	Shield->SetCheckpointRestoreInProgress(true, Shield->GetBindingGeneration());
	Poise->SetCheckpointRestoreInProgress(true, Poise->GetBindingGeneration());
	ASC->SetNumericAttributeBase(UNarrativeAttributeSetBase::GetShieldAttribute(), 30.f);
	ASC->SetNumericAttributeBase(UNarrativeAttributeSetBase::GetPoiseAttribute(), 40.f);
	Shield->ResetForCheckpoint();
	Poise->ResetForCheckpoint();
	Shield->SetCheckpointRestoreInProgress(false, OldShieldBinding);
	Poise->SetCheckpointRestoreInProgress(false, OldPoiseBinding);
	Fixture.Advance(4.f);
	...
	Shield->SetCheckpointRestoreInProgress(false, Shield->GetBindingGeneration());
	Poise->SetCheckpointRestoreInProgress(false, Poise->GetBindingGeneration());
