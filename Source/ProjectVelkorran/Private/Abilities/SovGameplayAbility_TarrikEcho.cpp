...
bool USovGameplayAbility_TarrikEchoBase::CheckCost(
...
void USovGameplayAbility_TarrikEchoBase::ApplyCost(
	const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo,
...
float USovGameplayAbility_TarrikEchoBase::GetCurrentEcho() const
{
	const USovEchoComponent* EchoComponent = GetEchoComponent();
	return IsValid(EchoComponent) ? EchoComponent->GetEcho() : 0.0f;
}

USovEchoComponent* USovGameplayAbility_TarrikEchoBase::GetEchoComponent() const
{
	return ResolveEchoComponent(CurrentActorInfo);
}

FGameplayAbilityTargetDataHandle USovGameplayAbility_TarrikEchoBase::GetAuthorityAimTargetData(
...
void USovGameplayAbility_TarrikEchoBase::FinishEchoAbility(const bool bWasCancelled)
{
	if (IsActive())
	{
		EndAbility(
...
USovEchoComponent* USovGameplayAbility_TarrikEchoBase::ResolveEchoComponent(
...
bool USovGameplayAbility_TarrikEchoBase::MeetsWeaponRequirement(
	const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo) const
{
	...
	if (WeaponFamily == ESovTarrikEchoWeaponFamily::Universal)
	{
...
void USovGameplayAbility_TarrikEchoBase::AddEchoFailureTags(
	FGameplayTagContainer* OptionalRelevantTags) const
{
	if (!OptionalRelevantTags)
	{
...
void USovGameplayAbility_TarrikEchoBase::BindCancellationTags(
	UAbilitySystemComponent* AbilitySystem)
{
	...
	// Close the race where a terminal/interrupt state arrived after activation
	// checks but before the callbacks were registered. One Busy contribution is
	// ours; a higher count means another system interrupted the committed cast.
...
void USovGameplayAbility_TarrikEchoBase::UnbindCancellationTags()
{
...
void USovGameplayAbility_TarrikEchoBase::HandleCancellationTagChanged(
...
void USovGameplayAbility_TarrikEchoBase::HandleMaximumDurationExpired()
{
	FinishEchoAbility(true);
}
